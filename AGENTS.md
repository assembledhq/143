...
- **`cmd/lint-schema`** — scans `migrations/*.up.sql` (down migrations are not scanned by design — they restore prior state). Every `CREATE TABLE` must declare `org_id uuid NOT NULL REFERENCES organizations(id)`. Exemptions (rare: root tables, FK-scoped child tables, infra tables) are declared in the `allowedNoOrgID` map in `cmd/lint-schema/main.go` with a short justification, or inline via `-- lint:no-org-id reason="..."` on the `CREATE TABLE` line (the `reason="..."` clause is required). Default to adding `org_id` over allowlisting; defense-in-depth beats transitive scoping.
- **`cmd/lint-stores`** — scans exported methods on `*XxxStore` types under `internal/db/`. Each method must either take an `orgID uuid.UUID` parameter (the name must end in `orgid` case-insensitively, e.g. `orgID`/`OrgID`/`org_id`/`srcOrgID`) or receive a `*models.X` carrier whose struct **literally declares an `OrgID` field** (the lint pre-scans `internal/models/*.go` to verify). Methods that are legitimately cross-org (pre-auth lookups, system cleanup, scheduler scans) opt out with a doc comment on the line above `func`:
