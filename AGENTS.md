...
**Multi-tenancy**: Every tenant-scoped table has an `org_id uuid NOT NULL` column. Every query MUST filter by `org_id`. Auth middleware extracts org from the session and sets it in request context. Missing an `org_id` filter is a data isolation bug. FKs are the default; hot append-only tables need an exception process because parent-row lock fan-in can become a Postgres operational risk. DB-backed `REFERENCES organizations(id)` foreign keys remain the ordinary path for product/control-plane tables, while very high-write append-only, event, log, cache, telemetry, and runtime-state tables may omit parent FKs only after explicit design review. See `docs/design/96-foreign-key-policy-and-hot-table-audit.md`.

Two lints enforce this, both run in CI via `make lint-tenancy`:

- **`cmd/lint-schema`** — scans `migrations/*.up.sql` (down migrations are not scanned by design — they restore prior state). Every `CREATE TABLE` must declare `org_id uuid NOT NULL REFERENCES organizations(id)` unless exempted. Schema-qualified (`public.foo`) and double-quoted (`"foo"`) table names are recognized and normalized for allowlist/display. Two exemption paths: (1) tables with no org_id at all — add to `allowedNoOrgID` in `cmd/lint-schema/main.go` with a justification, or use `-- lint:no-org-id reason="..."` anywhere in the CREATE TABLE statement; (2) reviewed hot append-only tables with `org_id NOT NULL` but intentionally no FK — use `-- lint:allow-hot-table-no-fk reason="..."` after documenting the exception and the write-path ownership check. Both `reason="..."` clauses are required. Default to the full FK form; defense-in-depth beats transitive scoping.
