...
**Multi-tenancy**: Every tenant-scoped table has an `org_id uuid NOT NULL` column. Every query MUST filter by `org_id`. Auth middleware extracts org from the session and sets it in request context. Missing an `org_id` filter is a data isolation bug. DB-backed `REFERENCES organizations(id)` foreign keys are the default for normal product/control-plane tables, but very high-write append-only, event, log, cache, telemetry, and runtime-state tables may omit parent FKs after an explicit design review. See `docs/design/future/96-foreign-key-policy-and-hot-table-audit.md`.

Two lints enforce this, both run in CI via `make lint-tenancy`:

- **`cmd/lint-schema`** — scans `migrations/*.up.sql` (down migrations are not scanned by design — they restore prior state). Every `CREATE TABLE` must declare `org_id uuid NOT NULL` unless it is genuinely cross-org/root/infra or transitively scoped. Schema-qualified (`public.foo`) and double-quoted (`"foo"`) table names are recognized and normalized for allowlist/display. Exemptions are declared in the `allowedNoOrgID` map in `cmd/lint-schema/main.go` with a short justification, or inline via `-- lint:no-org-id reason="..."` anywhere inside the `CREATE TABLE ( ... )` statement (header line or a dedicated comment line in the body). The `reason="..."` clause is required. Default to adding `org_id` over allowlisting; defense-in-depth beats transitive scoping. Default to `org_id uuid NOT NULL REFERENCES organizations(id)` for ordinary tables, but for reviewed hot tables keep `org_id uuid NOT NULL` and document why the FK is intentionally omitted.
