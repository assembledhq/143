...
func TestStaticEgressDeployWiring(t *testing.T) {
	...
	require.Contains(t, makefileText, "provision-egress:\n\t$(check-ssh-key)\n\t@deploy/scripts/sync-static-egress-secrets.sh --apply", "provision-egress should sync generated gateway and worker peer secrets before provisioning the gateway")
	...
	gatewayIndex := strings.Index(makefileText, `deploy/scripts/provision-egress.sh "" "$(SSH_KEY)"`)
