...
func TestStaticEgressDeployWiring(t *testing.T) {
	...
	require.Contains(t, deployText, "NEXT_PUBLIC_PREVIEW_ORIGIN_TEMPLATE=%s\\nSTATIC_EGRESS_ENABLED=%s\\nSTATIC_EGRESS_PUBLIC_IP=%s\\n", "app deploys should write static egress platform config for network status and preview routing")
	require.Contains(t, deployText, `if [ "${STATIC_EGRESS_ENABLED:-false}" = "true" ]; then`, "worker deploys should only pre-pull the static egress verifier when static egress is enabled")
	...
	require.Contains(t, provisionText, "NEXT_PUBLIC_PREVIEW_ORIGIN_TEMPLATE=%s\\nSTATIC_EGRESS_ENABLED=%s\\nSTATIC_EGRESS_PUBLIC_IP=%s\\n", "app provisioning should write static egress platform config for network status and preview routing")
