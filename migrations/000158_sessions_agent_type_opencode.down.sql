...
-- pre-migration-158 set. This rollback fails with an explicit message if any
...
BEGIN
    ...
    IF leftover_count > 0 THEN
        RAISE EXCEPTION
            'cannot roll back migration 000158: % session row(s) still reference agent_type = ''opencode''. Delete, archive, or reassign them before re-running the down migration.',
            leftover_count;
    END IF;
END$$;
