...
interface SplitRow {
  ...
  leftInlineHighlightRanges?: InlineDiffRange[];
  rightInlineHighlightRanges?: InlineDiffRange[];
}
...
function buildSplitRows(
  ...
  inlineRanges?: Map<number, InlineDiffRange[]>
): SplitRow[] {
  const rows: SplitRow[] = [];
  let i = 0;
  ...
  while (i < lines.length) {
    ...
    if (line.type === "context") {
      rows.push({
        ...
        leftInlineHighlightRanges: inlineRanges?.get(i),
        rightInlineHighlightRanges: inlineRanges?.get(i),
      });
      i++;
      ...
      for (let j = 0; j < maxLen; j++) {
        ...
        rows.push({
          ...
          leftInlineHighlightRanges: rm ? inlineRanges?.get(rm.idx) : undefined,
          rightInlineHighlightRanges: ad ? inlineRanges?.get(ad.idx) : undefined,
        });
      }
...
export function SplitDiffHunk({
  ...
  const rows = useMemo(
    () => buildSplitRows(hunk.lines, highlightedLines, inlineDiffRanges),
    [hunk.lines, highlightedLines, inlineDiffRanges]
  );
          ...
          <div key={i}>
            <div className="flex divide-x divide-border/50 [content-visibility:auto] [contain-intrinsic-size:auto_20px]">
              <SplitLineCell
                ...
                inlineHighlightRanges={row.leftInlineHighlightRanges}
                side="left"
                onAddComment={
              ...
              <SplitLineCell
                ...
                inlineHighlightRanges={row.rightInlineHighlightRanges}
                side="right"
                onAddComment={
