...
export function RecheckFromComment({ canManage }: { canManage: boolean }) {
  ...
  return <Dialog open={Boolean(assessmentID)} onOpenChange={(open) => { if (!open) close(); }}>
    <DialogContent>
      <DialogHeader><DialogTitle>Re-check PR</DialogTitle><DialogDescription>Check updated evidence using the previous code review. The current PR and review inputs determine whether a full review is needed.</DialogDescription></DialogHeader>
