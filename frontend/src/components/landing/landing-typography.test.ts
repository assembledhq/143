...
describe("landing typography", () => {
  it("keeps homepage product copy on a small shared type scale", () => {
    expect(landingTypography).toEqual({
      navBrand: "font-display text-base font-semibold tracking-tight",
      navAction: "text-base",
      button: "h-11 px-6 text-sm font-medium sm:h-11 has-[>svg]:px-6",
      eyebrow: "text-xs font-mono font-medium uppercase tracking-wider",
      heroTitle:
      ...
      sectionTitle:
        "font-display text-3xl font-semibold leading-tight tracking-tight sm:text-5xl",
      featureTitle:
        "font-display text-3xl font-semibold leading-tight tracking-tight sm:text-4xl",
      body: "text-sm leading-relaxed sm:text-base",
      cardTitle: "font-display text-base font-semibold tracking-tight",
      footerLink: "text-xs",
    });
  });
