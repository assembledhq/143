...
export function SessionDetailContent({ id }: { id: string }) {
  ...
  const handleEditableAgentTypeChange = useCallback((nextAgentType: string) => {
    ...
    // Only regenerate the label if it still looks autogenerated for the
    // current agent (e.g. "Codex 2"). Preserve a user-customized label.
    const currentAgent = AGENTS_BY_KEY[activeThread.agent_type];
    const looksAutogenerated = !!currentAgent && activeThread.label.startsWith(`${currentAgent.label} `);
