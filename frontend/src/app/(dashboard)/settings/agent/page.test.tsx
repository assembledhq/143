...
describe("Agent settings page", () => {
  ...
  it("hides auth type selection for API-key-only providers and shows Amp/Pi defaults", async () => {
    ...
    await user.keyboard("{Escape}");
	    ...
	    expect(within(dialog).getByLabelText("Default model")).toBeInTheDocument();
	    expect(within(dialog).getByText("Open-source models use audited US inference providers on OpenRouter; native OpenCode routes do not expose per-provider location controls.")).toBeInTheDocument();
	  ...
	  it("creates OpenCode auth with a detected backing provider and custom model override", async () => {
	    const user = userEvent.setup();
	  ...
	  it("detects an OpenRouter OpenCode key and saves the detected preset", async () => {
	    const user = userEvent.setup();
	    let capturedBody: Record<string, unknown> | null = null;
	    ...
	    await user.click(screen.getByLabelText("OpenCode"));
	    fireEvent.change(within(dialog).getByPlaceholderText("OpenCode or provider key"), {
	      target: { value: "sk-or-v1-opencode" },
	    });
