...
func TestStopFullReviewFallbackLoopPostgres(t *testing.T) {
	...
	tests := []struct {
		name, setup                             string
		stopped, pending, newActive, foreignOrg bool
	}{
...
CREATE TABLE code_review_requests(org_id uuid,pull_request_id uuid,assessment_id uuid,session_id uuid,status text);`)
			...
			for n := 1; n <= 3; n++ {
				...
				previous = &id
				current = models.CodeReviewAssessment{ID: id, OrgID: org, PullRequestID: pr, SessionID: session}
			}
			...
			got, err := stopFullReviewFallbackLoop(ctx, tx, current, 3, "restart limit reached", &state)
			if tt.foreignOrg {
