...
func TestLinearUserLinkStore_GetByLinearUserScopesByOrgAndWorkspace(t *testing.T) {
	...
	mock.ExpectQuery(`WHERE org_id = @org_id\s+AND linear_workspace_key = @linear_workspace_key\s+AND linear_user_id = @linear_user_id`).
		...
		WillReturnRows(pgxmock.NewRows(linearUserLinkColumns).AddRow(
			linkID, orgID, integrationID, &userID, "acme", "lin_user_1", &email, "Creator User",
			models.LinearUserLinkSourceEmailMatch, &now, now, now,
		))

	link, err := store.GetByLinearUser(context.Background(), orgID, "acme", "lin_user_1")
