...
func newSchedulingPostgres(t *testing.T) (*pgxpool.Pool, uuid.UUID, uuid.UUID, uuid.UUID) {
 ...
 CREATE TABLE session_threads(org_id uuid,session_id uuid,status text,id uuid DEFAULT gen_random_uuid(),cancel_requested_at timestamptz,completed_at timestamptz,last_activity_at timestamptz,failure_category text,failure_explanation text);
