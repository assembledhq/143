...
func (s *CodeReviewInsightStore) GetInsights(ctx context.Context, orgID uuid.UUID, filters models.CodeReviewInsightFilters) (models.CodeReviewInsights, error) {
	...
	if filters.RepositoryID != nil {
		...
		outcomeWhere += " AND o.repository_id = @repository_id"
		firstRequestWhere += " AND m.repository_id = @repository_id"
	}
	if filters.From != nil {
		args["from"] = *filters.From
		cohortWhere += " AND first_request.first_requested_at >= @from"
	}
	if filters.To != nil {
		args["to"] = *filters.To
		cohortWhere += " AND first_request.first_requested_at <= @to"
	}
	if filters.Decision != nil {
		args["decision"] = *filters.Decision
		outcomeWhere += " AND o.decision = @decision"
	}
	if filters.ReasonCode != nil {
		args["reason_code"] = *filters.ReasonCode
		outcomeWhere += " AND @reason_code = ANY(o.reason_codes)"
		disputeWhere += " AND @reason_code = ANY(d.contested_reason_codes)"
	}
	if filters.Direction != nil {
		args["direction"] = *filters.Direction
		disputeWhere += " AND d.direction = @direction"
	}
	query := `WITH first_requests AS MATERIALIZED (
		SELECT m.pull_request_id, min(m.created_at) AS first_requested_at
		FROM code_review_session_metadata m` + firstRequestWhere + `
		GROUP BY m.pull_request_id
	), outcomes AS MATERIALIZED (
		SELECT o.* FROM code_review_decision_outcomes o
		JOIN first_requests first_request ON first_request.pull_request_id = o.pull_request_id` + outcomeWhere + cohortWhere + `
	), disputes AS MATERIALIZED (
		SELECT d.* FROM code_review_decision_disputes d
		JOIN outcomes cohort ON cohort.org_id = d.org_id AND cohort.session_id = d.session_id` + disputeWhere + `
	), reason_decisions AS (
		SELECT reason_code, count(*)::bigint AS decisions
		FROM outcomes, unnest(reason_codes) reason_code GROUP BY reason_code
	), reason_disputes AS (
		SELECT reason_code, count(DISTINCT session_id)::bigint AS disputes
		FROM disputes, unnest(contested_reason_codes) reason_code GROUP BY reason_code
	), reassessment_attempts AS (
		SELECT d.*,
	...
	SELECT
		...
		(SELECT count(*)::bigint FROM disputes),
		COALESCE((SELECT count(DISTINCT session_id)::double precision FROM disputes) /
			NULLIF((SELECT count(*)::double precision FROM outcomes), 0), 0),
		(SELECT count(*)::bigint FROM disputes WHERE adjudication_status = 'upheld'),
		(SELECT count(*)::bigint FROM disputes WHERE reassessment_status = 'completed'),
		...
		(SELECT percentile_cont(0.5) WITHIN GROUP (ORDER BY EXTRACT(EPOCH FROM (m.completed_at - m.created_at)))
		 FROM outcomes o JOIN code_review_session_metadata m ON m.org_id = o.org_id AND m.session_id = o.session_id),
		(SELECT percentile_cont(0.5) WITHIN GROUP (ORDER BY EXTRACT(EPOCH FROM (adjudicated_at - created_at)))
		 FROM disputes WHERE adjudicated_at IS NOT NULL),
		(SELECT COALESCE(min(observed_until) FILTER (WHERE terminal = false), max(observed_until)) FROM outcomes),
		(SELECT max(projection_updated_at) FROM outcomes),
		COALESCE((SELECT jsonb_agg(jsonb_build_object('direction', direction, 'count', count) ORDER BY direction)
		 FROM (SELECT direction, count(*)::bigint AS count FROM disputes WHERE direction IS NOT NULL GROUP BY direction) x), '[]'),
		COALESCE((SELECT jsonb_agg(jsonb_build_object('kind', dispute_kind, 'count', count) ORDER BY count DESC, dispute_kind)
		 FROM (SELECT dispute_kind, count(*)::bigint AS count FROM disputes WHERE dispute_kind IS NOT NULL GROUP BY dispute_kind) x), '[]'),
		COALESCE((SELECT jsonb_agg(jsonb_build_object('policy_id', policy_id, 'policy_version', policy_version, 'decision', decision, 'count', count) ORDER BY policy_version, policy_id, decision)
		 FROM (SELECT o.policy_id, p.version AS policy_version, o.decision, count(*)::bigint AS count
			FROM outcomes o JOIN code_review_policies p ON p.org_id = o.org_id AND p.id = o.policy_id
			GROUP BY o.policy_id, p.version, o.decision) x), '[]'),
		COALESCE((SELECT jsonb_agg(jsonb_build_object('reason_code', COALESCE(rd.reason_code, rp.reason_code),
			'decisions', COALESCE(rd.decisions, 0), 'disputes', COALESCE(rp.disputes, 0),
			'dispute_rate', COALESCE(rp.disputes::double precision / NULLIF(rd.decisions, 0), 0))
			ORDER BY COALESCE(rd.decisions, 0) DESC, COALESCE(rd.reason_code, rp.reason_code))
		 FROM reason_decisions rd FULL JOIN reason_disputes rp USING (reason_code)), '[]'),
		COALESCE((SELECT jsonb_agg(jsonb_build_object(
			'reason_code', values.reason_code, 'actual', values.actual, 'limit', values.limit, 'count', values.count
		) ORDER BY values.reason_code, values.actual, values.limit)
		FROM (
			SELECT reason->>'code' AS reason_code,
				(reason->>'actual')::integer AS actual, (reason->>'limit')::integer AS limit,
				count(*)::bigint AS count
			FROM outcomes o
			JOIN code_review_session_metadata m ON m.org_id = o.org_id AND m.session_id = o.session_id
			CROSS JOIN LATERAL jsonb_array_elements(COALESCE(m.risk_reason_details, '[]'::jsonb)) reason
			WHERE reason ? 'actual' AND reason ? 'limit'
			GROUP BY reason->>'code', (reason->>'actual')::integer, (reason->>'limit')::integer
		) values), '[]'),
		COALESCE((SELECT jsonb_agg(jsonb_build_object(
			'attempt', bucket.attempt, 'input_change', bucket.input_change,
			'reassessments', bucket.reassessments, 'flips', bucket.flips
		) ORDER BY bucket.attempt, bucket.input_change)
		FROM (
			SELECT attempt,
				CASE
					WHEN semantic_input_hash_at_rerun IS NULL THEN 'unknown'
					WHEN semantic_input_hash_at_rerun = semantic_input_hash_at_filing THEN 'unchanged'
					ELSE 'changed'
				END AS input_change,
				count(*)::bigint AS reassessments,
				count(*) FILTER (WHERE reassessment_flipped = true)::bigint AS flips
			FROM reassessment_attempts GROUP BY attempt, input_change
		) bucket), '[]')`
	var insights models.CodeReviewInsights
	var directionsJSON, kindsJSON, mixJSON, reasonsJSON, actualVsLimitJSON, flipBucketsJSON []byte
	...
	if err != nil {
		return models.CodeReviewInsights{}, fmt.Errorf("query code review policy insights: %w", err)
	}
	for _, section := range []struct {
		...
		{"directions", directionsJSON, &insights.Directions},
		{"dispute kinds", kindsJSON, &insights.DisputeKinds},
		{"policy decision mix", mixJSON, &insights.PolicyDecisionMix},
		{"reasons", reasonsJSON, &insights.Reasons},
		{"actual versus limit", actualVsLimitJSON, &insights.ActualVsLimit},
		{"flip buckets", flipBucketsJSON, &insights.FlipBuckets},
	} {
		if err := json.Unmarshal(section.data, section.dst); err != nil {
			return models.CodeReviewInsights{}, fmt.Errorf("decode code review insights %s: %w", section.name, err)
		}
	}
	insights.RankingEnabled, err = s.RankingEnabled(ctx, orgID)
	if err != nil {
		return models.CodeReviewInsights{}, err
	}
	return insights, nil
}
