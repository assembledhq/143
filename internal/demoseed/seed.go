...
func ScanSeedSafety(body []byte) error {
	...
	for _, match := range emailPattern.FindAllString(text, -1) {
		if !strings.HasSuffix(strings.ToLower(match), "@143.dev") {
			return fmt.Errorf("non-preview email %q found in preview seed", match)
		}
	}
	for _, forbidden := range forbiddenSeedPatterns {
		if forbidden.pattern.Match(body) {
			return fmt.Errorf("%s-looking content found in preview seed", forbidden.name)
		}
	}
	return nil
}
...
func validateSeedURL(parsed *url.URL) error {
	...
	case "github.com":
		if parsed.RawQuery != "" || parsed.Fragment != "" {
			return fmt.Errorf("unapproved URL path %q found in preview seed", parsed.Path)
		}
		...
		return nil
	default:
		return fmt.Errorf("unapproved URL host %q found in preview seed", parsed.Hostname())
	}
}
...
func Check(ctx context.Context, opts CheckOptions) error {
	...
	if _, err := adminPool.Exec(ctx, "CREATE DATABASE "+quotedTempName); err != nil {
		return fmt.Errorf("create temporary preview seed database %q: %w", tempName, err)
	}
	defer func() {
		dropCtx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
		defer cancel()
		if _, dropErr := adminPool.Exec(dropCtx, "DROP DATABASE IF EXISTS "+quotedTempName); dropErr != nil {
			fmt.Fprintf(os.Stderr, "warning: failed to drop temporary preview seed database %s: %v\n", tempName, dropErr)
		}
	}()

	if err := RunMigrations(tempURL); err != nil {
		return fmt.Errorf("run migrations against temporary preview seed database: %w", err)
	}
	pool, err := connectPool(ctx, tempURL)
	if err != nil {
		return fmt.Errorf("connect temporary preview seed database: %w", err)
	}
	defer pool.Close()

	if err := ApplySeedSQL(ctx, pool, seedSQL); err != nil {
		return fmt.Errorf("apply preview seed first pass: %w", err)
	}
	if err := ApplySeedSQL(ctx, pool, seedSQL); err != nil {
		return fmt.Errorf("apply preview seed second pass: %w", err)
	}
...
func ApplySeedSQL(ctx context.Context, pool seedDB, seedSQL []byte) error {
	if _, err := pool.Exec(ctx, string(seedSQL)); err != nil {
		return fmt.Errorf("execute preview seed SQL: %w", err)
	}
	return nil
}
...
func AssertDemoSeedState(ctx context.Context, pool seedDB) error {
	assertions := []struct {
		...
		{
			name:     "demo users have preview passwords",
			query:    `SELECT count(*) FROM users WHERE org_id = '00000000-0000-4000-a000-000000000001'::uuid AND email IN ('preview-admin@143.dev', 'preview-member@143.dev', 'preview-builder@143.dev', 'preview-viewer@143.dev') AND password_hash IS NOT NULL`,
			expected: 4,
		},
		{
		...
		{
			name:     "demo reference document exists",
			query:    `SELECT count(*) FROM pm_documents WHERE org_id = '00000000-0000-4000-a000-000000000001'::uuid AND id = '00000000-0000-4000-a000-000000000860'::uuid AND source_type = 'manual' AND active = true`,
			expected: 1,
		},
		{
			name:     "demo reference document pin exists",
			query:    `SELECT count(*) FROM pm_document_set_pin_members WHERE pin_id = '00000000-0000-4000-a000-000000000864'::uuid AND document_id = '00000000-0000-4000-a000-000000000860'::uuid`,
			expected: 1,
		},
		{
			name:     "demo autogenerated PM document is absent",
			query:    `SELECT count(*) FROM pm_documents WHERE org_id = '00000000-0000-4000-a000-000000000001'::uuid AND id = '00000000-0000-4000-a000-000000000862'::uuid`,
			expected: 0,
		},
		{
			name:     "demo PM plans are absent",
		...
		{
			name:     "demo PM decisions are absent",
		...
		{
			name:     "demo project cycles are absent",
...
// rejects content that is unsafe for preview seed data.
...
func readSeedBody(seedPath string) ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("read preview seed %s: %w", seedPath, err)
	}
	if !info.IsDir() {
		body, err := os.ReadFile(seedPath) // #nosec G304 -- seedPath is an operator-supplied local seed file for developer preview tooling; contents are scanned before execution.
		if err != nil {
			return nil, fmt.Errorf("read preview seed %s: %w", seedPath, err)
		}
		return body, nil
	...
	if err != nil {
		return nil, fmt.Errorf("read preview seed directory %s: %w", seedPath, err)
	}
	...
	sort.Strings(names)
	if len(names) == 0 {
		return nil, fmt.Errorf("read preview seed directory %s: no .sql seed fragments found", seedPath)
	}
	...
	for i, name := range names {
		...
		if err != nil {
			return nil, fmt.Errorf("read preview seed fragment %s: %w", fragmentPath, err)
		}
		seed.Write(fragment)
