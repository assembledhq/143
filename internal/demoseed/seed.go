...
func AssertDemoSeedState(ctx context.Context, pool seedDB) error {
	assertions := []struct {
		...
		},
		{
			...
			query:    `SELECT count(*) FROM reference_documents WHERE org_id = '00000000-0000-4000-a000-000000000001'::uuid AND id = '00000000-0000-4000-a000-000000000860'::uuid AND source_type = 'manual' AND active = true`,
			expected: 1,
		},
		{
			name:     "demo reference document pin exists",
			query:    `SELECT count(*) FROM reference_context_set_pin_members WHERE pin_id = '00000000-0000-4000-a000-000000000864'::uuid AND reference_document_id = '00000000-0000-4000-a000-000000000860'::uuid`,
			expected: 1,
		},
		{
			name:     "demo autogenerated PM document is absent",
			query:    `SELECT count(*) FROM reference_documents WHERE org_id = '00000000-0000-4000-a000-000000000001'::uuid AND id = '00000000-0000-4000-a000-000000000862'::uuid`,
			expected: 0,
		},
		{
