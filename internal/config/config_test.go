...
func TestLoad_UsesDefaults(t *testing.T) {
	...
	t.Setenv("CODE_REVIEW_ASSESSMENTS_ENABLED", "")
	t.Setenv("CODE_REVIEW_RECHECKS_ENABLED", "")
	t.Setenv("CODE_REVIEW_DISPUTE_REASSESSMENTS_ENABLED", "")
	t.Setenv("CODE_REVIEW_DISPUTE_MAX_ACTIVE_REASSESSMENTS", "")
	...
	require.False(t, cfg.CodeReviewAssessmentsEnabled, "assessment capture should default disabled")
	require.False(t, cfg.CodeReviewRechecksEnabled, "recheck admission should default disabled")
...
func TestLoad_UsesEnvironmentOverrides(t *testing.T) {
	...
	t.Setenv("CODE_REVIEW_ASSESSMENTS_ENABLED", "true")
	t.Setenv("CODE_REVIEW_RECHECKS_ENABLED", "true")
	t.Setenv("CODE_REVIEW_DISPUTE_REASSESSMENTS_ENABLED", "false")
	t.Setenv("CODE_REVIEW_DISPUTE_MAX_ACTIVE_REASSESSMENTS", "25")
	...
	require.Equal(t, []string{"143-code-reviewer", "143-reviewer"}, cfg.CodeReviewAppReviewerLogins, "Load should split CODE_REVIEW_APP_REVIEWER_LOGINS from the environment")
	require.Equal(t, []string{"ai-reviewer", "bot-reviewer"}, cfg.CodeReviewAliasLogins, "Load should split CODE_REVIEW_ALIAS_LOGINS from the environment")
	require.Equal(t, []string{"ai-reviewers", "platform-review"}, cfg.CodeReviewTeamSlugs, "Load should split CODE_REVIEW_TEAM_SLUGS from the environment")
	require.True(t, cfg.CodeReviewAssessmentsEnabled, "Load should read assessment capture flag")
	require.True(t, cfg.CodeReviewRechecksEnabled, "Load should read recheck admission flag")
	require.False(t, cfg.CodeReviewDisputeReassessmentsEnabled, "Load should parse the dispute reassessment kill switch")
	require.Equal(t, 25, cfg.CodeReviewDisputeMaxActiveReassessments, "Load should parse the operator-wide active reassessment ceiling")
