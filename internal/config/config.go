...
type Config struct {
	...
	// SessionExecutorsEnabled moves long-running run_agent/continue_session
	// jobs into per-session executor containers. Default false for dark
	// rollout; worker deploys still use the active-session guardrail below.
	SessionExecutorsEnabled bool `env:"SESSION_EXECUTORS_ENABLED" envDefault:"false"`
