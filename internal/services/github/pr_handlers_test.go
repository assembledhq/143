...
func TestHandleAutoPreviewEvent_StartsForPolicyEnabledPullRequest(t *testing.T) {
	...
	previewMock.ExpectQuery("SELECT .+ FROM repository_preview_policies").
		...
		WillReturnRows(pgxmock.NewRows(githubRepositoryPreviewPolicyTestCols()).
			AddRow(uuid.New(), orgID, repoID, string(models.PreviewAutoModeWarm), string(models.PreviewSessionPrewarmModeOff), false, userID, now, now))
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			previewMock.ExpectQuery("SELECT .+ FROM repository_preview_policies").
				WithArgs(pgxmock.AnyArg(), pgxmock.AnyArg()).
				WillReturnRows(pgxmock.NewRows(githubRepositoryPreviewPolicyTestCols()).
					AddRow(uuid.New(), orgID, repoID, string(models.PreviewAutoModeOn), string(models.PreviewSessionPrewarmModeOff), false, userID, now, now))
	...
	}{
		{
			...
			setupMocks: func(t *testing.T, orgID uuid.UUID, repoMock, previewMock pgxmock.PgxPoolIface) {
				...
				previewMock.ExpectQuery("SELECT .+ FROM repository_preview_policies").
					WithArgs(pgxmock.AnyArg(), pgxmock.AnyArg()).
					WillReturnRows(pgxmock.NewRows(githubRepositoryPreviewPolicyTestCols()).
						AddRow(uuid.New(), orgID, repoID, string(models.PreviewAutoModeOff), string(models.PreviewSessionPrewarmModeOff), false, uuid.New(), now, now))
			},
		},
		{
