	...
	tests := []struct {
	...
	}{
		{
			...
			run: func(t *testing.T, mock pgxmock.PgxPoolIface, service *PRService, pr models.PullRequest, parentSession models.Session, userID uuid.UUID, now time.Time) {
				mock.ExpectQuery("SELECT .+ FROM sessions WHERE id").
					WithArgs(pgxmock.AnyArg(), pgxmock.AnyArg()).
					WillReturnRows(pgxmock.NewRows(prHealthSessionColumns).AddRow(newPRHealthSessionRow(parentSession.ID, pr.OrgID, now, models.SessionStatusCompleted)...))
				mock.ExpectBegin()
				mock.ExpectBegin()
...
func TestPRServiceCanResumeRepairSession(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			session: models.Session{Status: models.SessionStatusCompleted, SnapshotKey: &snapshotKey, SandboxState: "snapshotted"},
			want:    true,
		},
		{
			name:    "rejects running session",
			session: models.Session{Status: models.SessionStatusRunning, SnapshotKey: &snapshotKey, SandboxState: "snapshotted"},
			want:    false,
		},
	}
