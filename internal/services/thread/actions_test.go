...
func TestService_SummarizeSession(t *testing.T) {
	t.Parallel()
	...
	deps.threadStore.listBySessionFn = func(_ context.Context, _, _ uuid.UUID) ([]models.SessionThread, error) {
		return []models.SessionThread{
			{ID: threadA, SessionID: sessionID, OrgID: orgID, Label: "Codex", AgentType: models.AgentTypeCodex, Status: models.ThreadStatusRunning, CurrentTurn: 2, CostCents: 12.5},
			{ID: threadB, SessionID: sessionID, OrgID: orgID, Label: "Claude", AgentType: models.AgentTypeClaudeCode, Status: models.ThreadStatusIdle, CurrentTurn: 1, CostCents: 0},
		}, nil
	}
	fileEvents.listBySessionFn = func(_ context.Context, _, _ uuid.UUID, _ *time.Time) ([]models.SessionThreadFileEvent, error) {
		return []models.SessionThreadFileEvent{
			// shared.go is touched by both threads — should appear with
			// owner_thread_ids = [A, B] in the rollup.
			{OrgID: orgID, SessionID: sessionID, ThreadID: &threadA, Path: "shared.go", EventType: models.FileEventTypeModified, ObservedAt: now.Add(-1 * time.Minute)},
			{OrgID: orgID, SessionID: sessionID, ThreadID: &threadB, Path: "shared.go", EventType: models.FileEventTypeModified, ObservedAt: now},
			// codex_only.go touched only by A.
			{OrgID: orgID, SessionID: sessionID, ThreadID: &threadA, Path: "codex_only.go", EventType: models.FileEventTypeCreated, ObservedAt: now.Add(-2 * time.Minute)},
		}, nil
	}

	summary, err := svc.SummarizeSession(context.Background(), orgID, sessionID)
	require.NoError(t, err)
	require.Equal(t, sessionID, summary.SessionID)
	require.Equal(t, 1, summary.ActiveCount, "only the running thread counts as active")
	require.Len(t, summary.Threads, 2)

	// Threads come back in store order; the running one should expose its
	// touched paths so the side panel can render them.
	codex := summary.Threads[0]
	require.Equal(t, threadA, codex.ID)
	require.Equal(t, "Codex", codex.Label)
	require.Equal(t, "running", codex.Status)
	require.ElementsMatch(t, []string{"codex_only.go", "shared.go"}, codex.TouchedPaths)
	require.InDelta(t, 12.5, codex.CostCents, 0.001)

	// Touched files are rolled up newest-first; shared.go's most recent
	// observed_at is `now`, so it should appear before codex_only.go.
	require.Len(t, summary.TouchedFiles, 2)
	require.Equal(t, "shared.go", summary.TouchedFiles[0].Path)
	require.ElementsMatch(t, []uuid.UUID{threadA, threadB}, summary.TouchedFiles[0].OwnerThreadIDs, "shared file should attribute to both threads")
	require.Equal(t, "codex_only.go", summary.TouchedFiles[1].Path)
	require.ElementsMatch(t, []uuid.UUID{threadA}, summary.TouchedFiles[1].OwnerThreadIDs, "codex-only file should attribute to A only")
}

func TestService_SummarizeSession_NoFileEventStore(t *testing.T) {
	t.Parallel()
	...
	svc, deps := newTestService(t)
	// fileEvents intentionally not wired — summary should still come
	// back, just without per-tab touched paths or overlap rollup.
	...
	deps.threadStore.listBySessionFn = func(_ context.Context, _, _ uuid.UUID) ([]models.SessionThread, error) {
		return []models.SessionThread{
			{ID: threadID, SessionID: sessionID, OrgID: orgID, Label: "Codex", AgentType: models.AgentTypeCodex, Status: models.ThreadStatusIdle},
		}, nil
	}

	summary, err := svc.SummarizeSession(context.Background(), orgID, sessionID)
	require.NoError(t, err, "should degrade gracefully without a file-event store")
	require.Len(t, summary.Threads, 1)
	require.Empty(t, summary.TouchedFiles, "no events => no rollup")
	require.Empty(t, summary.Threads[0].TouchedPaths)
}

func TestService_SummarizeSession_SessionNotFound(t *testing.T) {
	t.Parallel()
	...
	_, err := svc.SummarizeSession(context.Background(), uuid.New(), uuid.New())
