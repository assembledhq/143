...
func TestReleaseInheritedContainer(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "the lease holder clears under the lock, commits, then destroys", ctx: context.Background(), session: models.Session{ContainerID: &container, WorkerNodeID: &node}, locked: true, wantDestroy: true, wantCleared: true},
		{name: "a destroy failure after the committed clear leaves the container to the reaper", ctx: context.Background(), session: models.Session{ContainerID: &container, WorkerNodeID: &node}, locked: true, destroyErr: errors.New("docker down"), wantDestroy: true, wantCleared: true},
