...
func TestCodexAdapter_Execute_ContinuationWithResumeSessionIDPassesHumanInputAnswer(t *testing.T) {
	...
	require.NoError(t, err, "continuation with explicit session ID should succeed")
	require.NotNil(t, result, "continuation should return a result")
	require.Contains(t, provider.ExecCalls[0], "Human input answer", "resume command should include the structured answer block")
	require.Contains(t, provider.ExecCalls[0], "toolu_01abc", "resume command should include the provider request id")
	require.Contains(t, provider.ExecCalls[0], "selected_choice_ids", "resume command should include selected choices")
	require.Contains(t, provider.ExecCalls[0], "approve", "resume command should include the selected approval")
}
