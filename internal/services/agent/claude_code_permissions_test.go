...
func TestClaudeCodePermissionModeForAuth(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:        "api key with supported current model and version enables auto",
		...
		{
			name:        "api key with future sonnet model enables auto without allowlist update",
			billingMode: TokenBillingModeAPIKey,
		...
		{
			name:        "api key with provider prefix and dated model enables auto",
			billingMode: TokenBillingModeAPIKey,
		...
		{
			name:        "api key with future opus model enables auto without allowlist update",
			billingMode: TokenBillingModeAPIKey,
		...
		{
			name:        "api key with older CLI keeps accept edits",
			billingMode: TokenBillingModeAPIKey,
			model:       models.ClaudeCodeModelSonnet46,
			version:     "2.0.99",
			expected:    ClaudeCodePermissionModeAcceptEdits,
		...
		{
			name:        "max subscription with supported model and version enables auto",
			billingMode: TokenBillingModeSubscription,
		...
		{
			name:        "unknown version keeps accept edits",
			billingMode: TokenBillingModeAPIKey,
			model:       models.ClaudeCodeModelSonnet46,
			expected:    ClaudeCodePermissionModeAcceptEdits,
		},
