...
func (p *countingProvider) StartPreview(ctx context.Context, sb *agent.Sandbox, cfg *models.PreviewConfig, opts StartPreviewOptions, observer ServiceObserver) (*PreviewHandle, error) {
	p.startCount++
	return p.mockProvider.StartPreview(ctx, sb, cfg, opts, observer)
}
...
func newPreviewInstanceRowWithRecycleSchedule(id, sessionID, orgID, userID uuid.UUID, status models.PreviewStatus, handle string, now time.Time, recycleScheduledAt *time.Time) []any {
	...
	setPreviewInstanceRowColumn(row, "recycle_scheduled_at", recycleScheduledAt)
	return row
}
