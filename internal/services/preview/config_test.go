...
func TestCommittedDogfoodFrontendScriptBindsExternally(t *testing.T) {
	...
	for {
		...
		if _, err := os.Stat(candidate); err == nil {
			...
			require.Contains(t, string(raw), "sh .143/preview-build-frontend.sh", "dogfood Next preview should rebuild through the shared build script when standalone output is missing")
			...
			require.Contains(t, string(buildScript), "npm run build", "dogfood build phase should produce a production bundle")
			require.Contains(t, string(buildScript), "sh .143/preview-install-frontend.sh", "dogfood build phase should install dependencies through the shared fallback")
			require.Contains(t, string(buildScript), "cp -R .next/static .next/standalone/frontend/.next/static", "dogfood build phase should stage generated CSS and static chunks next to the standalone server")
			require.Contains(t, string(buildScript), "cp -R public .next/standalone/frontend/public", "dogfood build phase should stage public assets next to the standalone server")
