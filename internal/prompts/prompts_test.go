...
{{ .Title }}`
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.Contains(t, orchestrator, "must not affect your evidence classification or `approval_recommended`", "orchestrator prompt should evaluate only code rather than external check status")
			require.Contains(t, orchestrator, `body="[P1] Explain the concrete scenario and fix."`, "orchestrator should require an eligible priority at the beginning of the inline comment body")
			require.Contains(t, orchestrator, "Only P0 and P1 issues may also become GitHub inline comments.", "orchestrator should restrict inline comments to blocking findings")
			require.Contains(t, orchestrator, "Do not emit `::code-comment` directives for P2 or P3 issues.", "orchestrator should suppress lower-priority inline comments")
			require.Contains(t, orchestrator, "P2 and P3 findings are advisory", "orchestrator should keep lower-priority findings from vetoing approval")
			require.Contains(t, orchestrator, "set a structured blocker boolean", "orchestrator should not translate advisory findings into another blocking signal")
			require.Contains(t, orchestrator, `"findings":`, "orchestrator output contract should preserve every severity as structured evidence")
			require.Contains(t, orchestrator, `"human_review_reasons":`, "orchestrator output contract should require explicit non-finding escalation reasons")
