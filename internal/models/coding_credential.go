...
// AnthropicSubscriptionAuthMode identifies which Claude Code subscription
// credential shape is stored in AnthropicSubscriptionConfig.
type AnthropicSubscriptionAuthMode string

const (
	// AnthropicSubscriptionAuthModeRotatingOAuth is the historical PKCE
	// access_token + refresh_token flow. Empty auth_mode rows are treated as
	// this mode for backwards compatibility with existing credentials.
	AnthropicSubscriptionAuthModeRotatingOAuth AnthropicSubscriptionAuthMode = "rotating_oauth"
	// AnthropicSubscriptionAuthModeSetupToken stores a long-lived
	// CLAUDE_CODE_OAUTH_TOKEN generated by `claude setup-token`.
	AnthropicSubscriptionAuthModeSetupToken AnthropicSubscriptionAuthMode = "setup_token"
)

// AnthropicSubscriptionConfig holds Claude Code OAuth credentials.
//
// Provider name is ProviderAnthropicSubscription. Empty AuthMode is the
// historical rotating OAuth flow for compatibility with existing rows.
type AnthropicSubscriptionConfig struct {
	AuthMode AnthropicSubscriptionAuthMode `json:"auth_mode,omitempty"`

	// Rotating OAuth fields from the PKCE flow.
	...
	// Setup-token fields from `claude setup-token`.
	OAuthToken          string    `json:"oauth_token,omitempty"` // #nosec G117 -- JSON config field
...
func (c AnthropicSubscriptionConfig) IsExpired() bool {
	if c.IsSetupToken() {
		return c.OAuthTokenExpiresAt.IsZero() || time.Now().After(c.OAuthTokenExpiresAt)
	}
...
// IsSetupToken returns true for long-lived CLAUDE_CODE_OAUTH_TOKEN rows.
func (c AnthropicSubscriptionConfig) IsSetupToken() bool {
	return c.AuthMode == AnthropicSubscriptionAuthModeSetupToken
}
...
// Validate ensures the row carries either active rotating OAuth tokens, a
// setup token, or a pending PKCE handshake.
func (c AnthropicSubscriptionConfig) Validate() error {
	switch c.AuthMode {
	case "", AnthropicSubscriptionAuthModeRotatingOAuth:
	case AnthropicSubscriptionAuthModeSetupToken:
		if c.OAuthToken == "" {
			return errors.New("oauth_token is required for setup-token auth")
		}
		if c.OAuthTokenExpiresAt.IsZero() {
			return errors.New("oauth_token_expires_at is required for setup-token auth")
		}
		if !time.Now().Before(c.OAuthTokenExpiresAt) {
			return errors.New("oauth_token_expires_at must be in the future")
		}
		return nil
	default:
		return fmt.Errorf("unsupported anthropic subscription auth_mode %q", c.AuthMode)
	}
