...
func TestPreviewDiagnosticReadsAreViewerReadableButMutationsAreBuilderOnly(t *testing.T) {
	...
	sessionObserveRoute := strings.Index(string(source), `r.Post("/api/v1/sessions/{id}/preview/observe", previewHandler.Observe)`)
	sessionWatchRoute := strings.Index(string(source), `r.Post("/api/v1/sessions/{id}/preview/watch", previewHandler.WatchBrowser)`)
	sessionActRoute := strings.Index(string(source), `r.Post("/api/v1/sessions/{id}/preview/act", previewHandler.Act)`)
	controlReadRoute := strings.Index(string(source), `r.Get("/api/v1/sessions/{id}/preview/control", previewHandler.GetBrowserControl)`)
	humanActRoute := strings.Index(string(source), `r.Post("/api/v1/sessions/{id}/preview/control/act", previewHandler.HumanAct)`)
	...
	require.NotEqual(t, -1, sessionObserveRoute, "session observe route should be registered")
	require.NotEqual(t, -1, sessionWatchRoute, "session watch route should be registered")
	require.NotEqual(t, -1, sessionActRoute, "session act route should be registered")
	require.NotEqual(t, -1, controlReadRoute, "browser control read route should be registered")
	require.NotEqual(t, -1, humanActRoute, "human browser input route should be registered")
	...
	require.Greater(t, sessionScreenshotRoute, builderGroupStart, "session screenshot should require builder workflow access because it persists outputs")
	...
	require.Greater(t, sessionWatchRoute, readGroupStart, "side-effect-free session watch should be available to preview readers")
	require.Less(t, sessionWatchRoute, builderGroupStart, "session watch should not require builder workflow access")
	require.Greater(t, sessionObserveRoute, builderGroupStart, "configurable session observe should require builder workflow access")
	require.Greater(t, sessionActRoute, builderGroupStart, "session act should require builder workflow access because it drives the app")
	require.Greater(t, controlReadRoute, readGroupStart, "browser control state should be viewer readable")
	require.Less(t, controlReadRoute, builderGroupStart, "browser control state should not require mutation access")
	require.Greater(t, humanActRoute, builderGroupStart, "human browser input should require builder workflow access")
	require.Greater(t, previewScreenshotRoute, builderGroupStart, "preview-id screenshot should require builder workflow access because it persists outputs")
...
func TestSandboxPreviewRoutesUseInternalSessionScope(t *testing.T) {
	t.Parallel()
	source, err := os.ReadFile("router.go")
	...
	require.NotEqual(t, -1, controlRoute, "sandbox control-state route should be registered")
	require.NotEqual(t, -1, handoffRoute, "sandbox handoff route should be registered")
}
