...
func NewRouter(cfg *config.Config, pool *pgxpool.Pool, logger zerolog.Logger, sentryReporter observability.Reporter, codexAuthSvc *codexauth.Service, claudeCodeAuthSvc *claudecodeauth.Service, llmClient llm.Client, fileReader sandbox.FileReader, canceller handlers.SessionCanceller, threadCanceller *agent.ThreadCancelRegistry, previewProvider preview.PreviewCapableProvider, snapshotExecutor preview.SnapshotExecutor, sandboxProvider agent.SandboxProvider, sandboxCapacity *agent.SandboxCapacityGate, snapshotStore storage.SnapshotStore, orgSettingsInvalidator handlers.OrgSettingsInvalidator, shutdownCh <-chan struct{}, redisClient *cache.Client, sessionStreams *cache.SessionStreams, sharedCodingCredentialStore ...*db.CodingCredentialStore) (*chi.Mux, *http.Server, *preview.RecycleWorker, io.Closer, *preview.Manager, error) {
	...
	automationOutcomeStore := db.NewAutomationOutcomeStore(pool)
	...
	automationHandler.SetOutcomeStore(automationOutcomeStore)
	...
	automationOutcomeService := automations.NewOutcomeService(automationRunStore, automationOutcomeStore)
	...
	apiRoutes.Group(func(r chi.Router) {
		...
		internalAutomationOutcomeHandler := handlers.NewInternalAutomationOutcomeHandler(automationOutcomeService, sessionStore, cfg.SessionSecret, logger)
		...
		r.Route("/api/v1/internal", func(r chi.Router) {
			...
			r.Patch("/automations/{id}", internalAutomationHandler.Update)
			r.Post("/automations/{id}/run", internalAutomationHandler.RunNow)
			r.Post("/automations/{id}/pause", internalAutomationHandler.Pause)
			r.Post("/automations/{id}/resume", internalAutomationHandler.Resume)
			...
			r.Post("/automation-goal-improvements/{improvement_id}/complete", internalAutomationGoalImprovementHandler.Complete)
			r.Post("/automation-runs/current/outcome", internalAutomationOutcomeHandler.Report)
		...
		r.Group(func(r chi.Router) {
			...
			r.Group(func(r chi.Router) {
				...
				r.Get("/api/v1/automations/{id}/decisions", automationHandler.ListDecisions)
				r.Get("/api/v1/automations/{id}/decision-stats", automationHandler.DecisionStats)
