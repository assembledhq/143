...
func NewRouter(cfg *config.Config, pool *pgxpool.Pool, logger zerolog.Logger, sentryReporter observability.Reporter, codexAuthSvc *codexauth.Service, claudeCodeAuthSvc *claudecodeauth.Service, llmClient llm.Client, fileReader sandbox.FileReader, canceller handlers.SessionCanceller, threadCanceller *agent.ThreadCancelRegistry, previewProvider preview.PreviewCapableProvider, snapshotExecutor preview.SnapshotExecutor, sandboxProvider agent.SandboxProvider, sandboxCapacity *agent.SandboxCapacityGate, snapshotStore storage.SnapshotStore, orgSettingsInvalidator handlers.OrgSettingsInvalidator, shutdownCh <-chan struct{}, redisClient *cache.Client, sessionStreams *cache.SessionStreams, sharedDependencies ...RouterSharedDependencies) (*chi.Mux, *http.Server, *preview.RecycleWorker, io.Closer, *preview.Manager, error) {
	...
	codeReviewDisputeSvc := codereviewsvc.NewDisputeService(
	...
	apiRoutes.Group(func(r chi.Router) {
		...
		r.Route("/api/v1/internal", func(r chi.Router) {
			...
			r.Post("/sessions/{id}/preview/update", toolGate.Require("preview:update", internalAgentPreviewHandler.Update))
			r.Post("/sessions/{id}/preview/restart", toolGate.Require("preview:restart", internalAgentPreviewHandler.Restart))
			r.Delete("/sessions/{id}/preview", toolGate.Require("preview:stop", internalAgentPreviewHandler.Stop))
			r.Post("/sessions/{id}/preview/observe", toolGate.Require("preview:observe", internalAgentPreviewHandler.Observe))
			r.Post("/sessions/{id}/preview/act", toolGate.Require("preview:act", internalAgentPreviewHandler.Act))
			r.Get("/sessions/{id}/preview/control", toolGate.Require("preview:control", internalAgentPreviewHandler.BrowserControl))
			r.Post("/sessions/{id}/preview/control/request-handoff", toolGate.Require("preview:request_handoff", internalAgentPreviewHandler.RequestHumanHandoff))
			r.Post("/sessions/{id}/preview/screenshot", toolGate.Require("preview:screenshot", internalAgentPreviewHandler.Screenshot))
			r.Get("/sessions/{id}/preview/console", toolGate.Require("preview:console", internalAgentPreviewHandler.Console))
			r.Post("/sessions/{id}/preview/inspect", toolGate.Require("preview:inspect", internalAgentPreviewHandler.Inspect))
			r.Post("/sessions/{id}/preview/interact", toolGate.Require("preview:interact", internalAgentPreviewHandler.Interact))
			r.Post("/sessions/{id}/preview/multi-viewport", toolGate.Require("preview:multi_viewport", internalAgentPreviewHandler.MultiViewport))
			r.Post("/sessions/{id}/preview/visual-diff", toolGate.Require("preview:visual_diff", internalAgentPreviewHandler.VisualDiff))
			r.Post("/sessions/{id}/preview/assert", toolGate.Require("preview:assert", internalAgentPreviewHandler.Assert))
			r.Post("/issues", toolGate.Require("issue:create", internalIssueHandler.Create))
			r.Post("/session/pr", toolGate.Require("pr:create", internalPullRequestHandler.Create))
			r.Patch("/session/pr", toolGate.Require("pr:update", internalPullRequestHandler.Update))
			r.Post("/sessions/{sessionID}/pr", toolGate.Require("pr:create", internalPullRequestHandler.Create))
			r.Patch("/sessions/{sessionID}/pr", toolGate.Require("pr:update", internalPullRequestHandler.Update))
			r.Get("/sessions/{id}/changesets", toolGate.Require("changesets:list", internalChangesetHandler.List))
			r.Get("/sessions/{id}/changesets/split-status", toolGate.Require("changesets:status", internalChangesetHandler.Status))
			r.Post("/sessions/{id}/changesets", toolGate.Require("changesets:create", internalChangesetHandler.Create))
			r.Post("/sessions/{id}/changesets/{changeset_id}/materialize", toolGate.Require("changesets:materialize", internalChangesetHandler.Materialize))
			r.Post("/sessions/{id}/changesets/{changeset_id}/publish", toolGate.Require("changesets:publish", internalChangesetHandler.Publish))
			r.Post("/sessions/{id}/changesets/publish-stack", toolGate.Require("changesets:publish_stack", internalChangesetHandler.PublishStack))
			r.Post("/sessions/{id}/changesets/{changeset_id}/restack-descendants", toolGate.Require("changesets:restack", internalChangesetHandler.Restack))
			r.Post("/sessions/{id}/changesets/{changeset_id}/import-remote", toolGate.Require("changesets:import_remote", internalChangesetHandler.ImportRemote))
			...
			r.Post("/automations/{id}/run", toolGate.Require("automation:run", internalAutomationHandler.RunNow))
			r.Post("/automations/{id}/pause", toolGate.Require("automation:pause", internalAutomationHandler.Pause))
			r.Post("/automations/{id}/resume", toolGate.Require("automation:resume", internalAutomationHandler.Resume))
			r.Post("/eval/candidates", toolGate.Require("eval:add", internalEvalHandler.AddCandidate))
			r.Post("/evals/bootstrap/{bootstrap_run_id}/candidates", toolGate.Require("eval:add", internalEvalHandler.AddCandidate))
			r.Post("/automation-goal-improvements/{improvement_id}/complete", toolGate.Require("automation-goal-improvement:complete", internalAutomationGoalImprovementHandler.Complete))
			r.Get("/agent-capabilities/effective", toolGate.Require("capability:list", internalAgentCapabilitiesHandler.Effective))
			r.Post("/agent-capabilities/requests", toolGate.Require("capability:request", internalAgentCapabilitiesHandler.Request))
			r.Get("/session-history/search", toolGate.Require("session-history:search", internalSessionHistoryHandler.Search))
			r.Get("/session-history/{session_id}", toolGate.Require("session-history:get", internalSessionHistoryHandler.Get))
			r.Get("/session-history/{session_id}/threads/{thread_id}/messages", toolGate.Require("session-history:messages", internalSessionHistoryHandler.Messages))
			r.Get("/code-reviews", toolGate.Require("code-review-history:list", internalCodeReviewHandler.List))
			r.Get("/code-reviews/policy", toolGate.Require("code-review-history:policy", internalCodeReviewHandler.Policy))
			r.Put("/code-reviews/policy", toolGate.Require("code-review-history:update_policy", internalCodeReviewHandler.UpdatePolicy))
			r.Get("/code-reviews/policies/{policy_id}", toolGate.Require("code-review-history:policy", internalCodeReviewHandler.PolicyByID))
			r.Get("/code-reviews/{session_id}", toolGate.Require("code-review-history:get", internalCodeReviewHandler.Get))
			r.Get("/session-tabs", toolGate.Require("session-tabs:list", internalSessionTabsHandler.List))
			r.Post("/session-tabs", toolGate.Require("session-tabs:create", internalSessionTabsHandler.Create))
			r.Get("/session-tabs/{thread_id}", toolGate.Require("session-tabs:get", internalSessionTabsHandler.Get))
			r.Get("/session-tabs/{thread_id}/messages", toolGate.Require("session-tabs:messages", internalSessionTabsHandler.Messages))
			r.Post("/session-tabs/{thread_id}/messages", toolGate.Require("session-tabs:send", internalSessionTabsHandler.SendMessage))
		})
		...
		r.Group(func(r chi.Router) {
			...
			r.Group(func(r chi.Router) {
				...
				r.With(goalImproveLimit).Post("/api/v1/automations/{id}/goal-improvements", automationHandler.CreateSavedGoalImprovement)
				r.With(goalImproveApplyLimit).Post("/api/v1/automations/goal-improvements/{improvement_id}/cancel", automationHandler.CancelGoalImprovement)
			...
			r.Group(func(r chi.Router) {
				...
				r.Get("/api/v1/code-review-policies/versions", codeReviewHandler.ListPolicyVersions)
