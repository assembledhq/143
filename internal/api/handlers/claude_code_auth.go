...
// claudeCodeOAuthTokenMax bounds pasted setup-token values. The token is
// generated by `claude setup-token`; this cap keeps request bodies and logs
// bounded without assuming a specific token encoding.
const claudeCodeOAuthTokenMax = 8192
...
// the user's local `claude setup-token` command.
func (h *ClaudeCodeAuthHandler) StoreOAuthToken(w http.ResponseWriter, r *http.Request) {
	orgID := middleware.OrgIDFromContext(r.Context())
	user := middleware.UserFromContext(r.Context())
	if user == nil {
		writeError(w, r, http.StatusUnauthorized, "UNAUTHENTICATED", "unauthenticated", nil)
	...
	body.Label = strings.TrimSpace(body.Label)
	body.OAuthToken = strings.TrimSpace(body.OAuthToken)
	if body.Label == "" {
		writeError(w, r, http.StatusBadRequest, "INVALID_LABEL", "label is required", nil)
		return
	}
	...
	if body.OAuthToken == "" {
		writeError(w, r, http.StatusBadRequest, "INVALID_TOKEN", "Claude Code OAuth token is required", nil)
		return
	}
	if len(body.OAuthToken) > claudeCodeOAuthTokenMax {
		writeError(w, r, http.StatusBadRequest, "INVALID_TOKEN", fmt.Sprintf("Claude Code OAuth token must be %d characters or fewer", claudeCodeOAuthTokenMax), nil)
		return
	}
	...
	createdBy := user.ID
	resp, err := h.svc.StoreOAuthToken(r.Context(), scope, &createdBy, body.Label, body.OAuthToken)
	if err != nil {
		var labelErr *db.ErrCredentialLabelTaken
		if errors.As(err, &labelErr) {
			writeError(w, r, http.StatusConflict, "LABEL_TAKEN", labelErr.Error(), err)
		...
		if errors.Is(err, claudecodeauth.ErrInvalidOAuthToken) {
			writeError(w, r, http.StatusBadRequest, "INVALID_TOKEN", "Claude Code OAuth token is invalid or expired", err)
			return
		}
		writeError(w, r, http.StatusInternalServerError, "AUTH_TOKEN_STORE_FAILED", "failed to store Claude Code OAuth token", err)
		return
	}
