...
func TestPMHandler_AcceptRefresh(t *testing.T) {
	...
	// Update now uses insert-only versioning: inner savepoint + deactivate + insert.
	mock.ExpectBegin() // savepoint
	mock.ExpectQuery("UPDATE pm_documents SET active = false").
		WithArgs(pgxmock.AnyArg(), pgxmock.AnyArg()).
		WillReturnRows(pgxmock.NewRows([]string{"logical_id", "content_hash", "title"}).AddRow(activeID, "", "PM Context"))
