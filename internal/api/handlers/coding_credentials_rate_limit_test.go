...
func TestCodingCredentialCheckRateLimit(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			handler.SetRateLimitChecker(rateLimitCheckerFunc(func(_ context.Context, scope models.Scope, actualID uuid.UUID) error {
				called = true
			...
			r := withUserAndOrg(httptest.NewRequest(http.MethodPost, "/api/v1/coding-credentials/"+id.String()+"/check-rate-limit?scope="+tt.scope, nil), userID, orgID, tt.role)
