...
func TestProjectAnalysisHandler_Improve_DesignTarget(t *testing.T) {
	t.Parallel()
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/projects/"+projectID.String()+"/ai/improve", bytes.NewBufferString(`{"target":"design"}`))
	req = req.WithContext(middleware.WithOrgID(req.Context(), orgID))
	req = req.WithContext(withProjectRouteParam(req.Context(), projectID))
	rr := httptest.NewRecorder()
	...
	require.Contains(t, rr.Body.String(), "Add design references")
	require.NoError(t, mock.ExpectationsWereMet())
}

func TestProjectAnalysisHandler_Improve_TasksTarget(t *testing.T) {
	t.Parallel()
	...
	mock.ExpectQuery("SELECT .+ FROM project_specs WHERE project_id").
		...
		WillReturnRows(
			pgxmock.NewRows(projectSpecHandlerColumns).
				AddRow(newSpecRow(specID, projectID, orgID, "prd", "A detailed spec content here.", now)...),
		)
	mock.ExpectQuery("SELECT .+ FROM project_attachments WHERE project_id").
		WithArgs(pgxmock.AnyArg(), pgxmock.AnyArg()).
		WillReturnRows(pgxmock.NewRows(projectAttachmentHandlerColumns))
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/projects/"+projectID.String()+"/ai/improve", bytes.NewBufferString(`{"target":"tasks"}`))
	req = req.WithContext(middleware.WithOrgID(req.Context(), orgID))
	req = req.WithContext(withProjectRouteParam(req.Context(), projectID))
	rr := httptest.NewRecorder()
	...
	require.Equal(t, http.StatusOK, rr.Code)
	require.Contains(t, rr.Body.String(), "Break specs into tasks")
	require.NoError(t, mock.ExpectationsWereMet())
}
