...
func (h *PMHandler) AcceptRefresh(w http.ResponseWriter, r *http.Request) {
	...
	details := pmDocumentAuditSnapshot(&refreshDoc)
	details["active_document_id"] = activeDoc.ID.String()
	details["changes"] = map[string]any{
		"active_content_hash":   auditChange(beforeActiveDoc.ContentHash, refreshDoc.ContentHash),
		"active_content_length": auditChange(len(beforeActiveDoc.Content), len(refreshDoc.Content)),
	}
	emitUserAudit(h.audit, r, models.AuditActionPMRefreshAccepted, models.AuditResourcePMDocument, &refreshIDStr,
...
func (h *PMHandler) RejectRefresh(w http.ResponseWriter, r *http.Request) {
	...
	refreshIDStr := refreshID.String()
	emitUserAudit(h.audit, r, models.AuditActionPMRefreshRejected, models.AuditResourcePMDocument, &refreshIDStr,
		marshalAuditDetails(*zerolog.Ctx(r.Context()), pmDocumentAuditSnapshot(&doc)))
