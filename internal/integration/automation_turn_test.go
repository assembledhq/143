...
func TestAutomationTurn_AttemptFenceHelpers(t *testing.T) {
	...
	sessionID := h.outcome.SessionID
	updated, err := h.store.EndInterruptedAttempt(ctx, h.orgID, h.run.ID, sessionID, uuid.New(), models.SessionStatusIdle)
	require.NoError(t, err, "interrupted end with another token")
	require.False(t, updated, "another token cannot end the attempt")
	require.NotEqual(t, models.SessionStatusIdle, h.session(t).Status, "the session is untouched")
	...
	updated, err = h.store.EndInterruptedAttempt(ctx, h.orgID, h.run.ID, sessionID, h.lockToken, models.SessionStatusIdle)
	require.NoError(t, err, "interrupted end after reclaim")
	require.False(t, updated, "a reclaimed job no longer ends the attempt")
	require.NotEqual(t, models.SessionStatusIdle, h.session(t).Status, "the session is untouched after the reclaim")
	...
	updated, err = h.store.EndInterruptedAttempt(ctx, h.orgID, h.run.ID, sessionID, h.lockToken, models.SessionStatusIdle)
	require.NoError(t, err, "interrupted end")
	require.True(t, updated, "the lease holder ends the interrupted attempt")
	require.Equal(t, models.SessionStatusIdle, h.session(t).Status, "the session is restored in the same fenced statement")
	_, err = h.results.GetByRun(ctx, h.orgID, h.run.ID)
