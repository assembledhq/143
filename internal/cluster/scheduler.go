...
func (s *Scheduler) scheduleContextRefreshes(ctx context.Context, orgIDs []uuid.UUID, orgSettings map[uuid.UUID]models.OrgSettings, now time.Time) {
	// Retained as a compatibility seam until the PM scheduler is deleted.
	// Context refresh is disabled even if an old internal caller invokes it.
	const contextRefreshEnabled = false
	if !contextRefreshEnabled {
		return
	}
	if s.pmDocs == nil {
		return
