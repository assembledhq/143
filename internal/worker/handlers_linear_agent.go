...
func newLinearAgentEventHandler(deps LinearAgentEventHandlerDeps) JobHandler {
	...
	return func(ctx context.Context, jobType string, raw json.RawMessage) error {
		...
		if payload.Action != "created" {
			// `prompted` lands in phase 3; until then the dispatcher
			// short-circuits before enqueueing. Any other action is a
			// future Linear addition we don't yet support.
			logger.Info().Str("action", payload.Action).Msg("linear_agent_event: action not implemented in phase 2; skipping")
			return nil
		}
