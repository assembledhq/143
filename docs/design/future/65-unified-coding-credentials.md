...
> - `migrations/000106_coding_credentials.{up,down}.sql` — unified table, four partial indexes, `coding_credentials_migrations` sentinel.
> - `migrations/000107_copy_coding_credentials.{up,down}.sql` — idempotent SQL data copy from `org_credentials` (with `openai_chatgpt → openai_subscription` rename) and `user_credentials` (team-default rows promoted to org-scoped).
...
> 1. Apply migrations `000106` and `000107`. The legacy tables are untouched.
...
> 4. Rollback: `make migrate-down` reverses 000107 (deletes from `coding_credentials`) and 000106 (drops the table). Legacy data is intact.
