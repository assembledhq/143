...
- **Epochs.** `automation_targets` keeps `observed_head_sha`, `observed_head_updated_at`, `head_epoch`, and `head_resolution_pending`. An epoch is assigned only to an **authoritative** head: a delivery whose `updated_at` is strictly newer than `observed_head_updated_at`, or a successful dispatch-time lookup. At arrival of a run that carries the PR head and its timestamp (`synchronize`, `opened`, `reopened`, `ready_for_review`), inside the target-locked arrival transaction:
  - strictly newer timestamp: the target adopts the delivered head, increments `head_epoch`, and the run is stamped `head_resolution = authoritative` with that epoch; any older waiting push run is superseded;
  - strictly older timestamp with a different head: the run is skipped at arrival as `stale_head`; it can never supersede newer work;
  - equal timestamp with a different head, or a missing timestamp: the run is **ambiguous**. It waits with `head_resolution = ambiguous`, a null epoch, and neither supersedes nor is superseded; the target sets `head_resolution_pending = true`;
  - same head as observed: the run joins the observed epoch.
  Divergent force-pushed heads are ordered the same way, because the epoch, not ancestry, decides which is newer.
- **Non-push runs** (comments, reviews, checks, `edited`, `converted_to_draft`, `labeled`, `merged`) keep their delivered head. At reservation they receive the target's current epoch only when that head equals `observed_head_sha`; otherwise their epoch is null. A run delivered without a head is enriched at dispatch by the lookup, reviews the current head, and receives its epoch. Null epochs never advance the baseline, so a comment on an older head that waits behind a newer push cannot move the baseline back when it completes.
- **At dispatch** of a push run, the handler looks up the PR's current head and `updated_at` from GitHub with the installation token, as `PRService` already does for `issue_comment` enrichment. On success: a head newer than observed (a missed webhook) is adopted with a new epoch and the run reviews it; if ambiguous candidates exist, the candidate whose head equals the current head is stamped authoritative with the epoch and the other ambiguous push runs are superseded, and `head_resolution_pending` is cleared. On failure with no ambiguity pending: the run reviews its delivered head, which is safe because the surviving push run carries the newest strictly-ordered head, and the run sets `head_lookup_degraded`. On failure with ambiguity pending: the run is not executed; the job retries with backoff (30 s doubling to 10 min) inside the wait timeout, and `ReconcileTargetWakes` retries the lookup for targets with `head_resolution_pending`. If the wait timeout arrives unresolved, the ambiguous candidates execute in arrival order, each at its own delivered head with `head_resolution = unresolved` and a null epoch, so nothing is discarded; the cost is at most one redundant review per tie.
