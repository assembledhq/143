...
- **At-least-once execution with scoped delivery protection.** The default tool allowlist contains no external writes. The opt-in `automation_actions` exception executes independently selected actions through durable reservations and receipts; confirmed or uncertain sends cannot be blindly repeated. Shell and network effects inside the sandbox remain at-least-once; this design does not claim otherwise.
...
The scheduler's stuck-run reaper marks pending or running runs failed one hour after `triggered_at` (`stuckAutomationRunThreshold`). That is wrong for a per-target run, and terminalizing the run row alone is worse than leaving it: an executing run holds a session, a primary thread, and possibly the target's pending ownership release, none of which that query releases. The reaper therefore skips every run that entered per-target dispatch (`dispatch_state IS NOT NULL`), and the per-target recovery sweep owns them:

- a waiting run is not stuck; it is queued behind its target and has its own two-hour wait timeout;
- an executing run is **abandoned** when no live lease remains (no running job with an unexpired lease, and no active `session_executors` row) and either its job reached a terminal state or its `attempt_started_at` is older than one hour. The sweep completes an abandoned run from its result marker when one exists for the current attempt, and otherwise records `retries_exhausted`; both paths release the session, the primary thread, and any pending ownership release, and wake the target.
...
Every path that starts a turn on an owned session, or destroys what its next turn restores from, returns 409 `SESSION_AUTOMATION_OWNED`: thread message sends, thread creation, follow-up commands, preview start and its ensure, restart, and update siblings (which take the same container), the review-comment send that claims the session and enqueues its own continuation, the human-input answer and cancel that do the same, retry and start-over, archive (which deletes the checkpoint), PR worktree materialization, and split verification. Cancelling a turn is deliberately **not** guarded: a cancelled automation turn is a documented outcome with its own result marker, so cancel and Reset are the two ways to interrupt a target. Read routes are unaffected. with `details.automation_id`, `details.target_id`, `details.reset_url`, and `details.release_pending` (true when the generation is already retired and the marker will clear when the current turn finishes). Read routes are unaffected.
...
| Capabilities | The allowlist is exact: `update_policy`, automation management actions, eval add, PR create, Slack send, Linear and PagerDuty writes, goal-improvement complete, and preview actions are denied server-side even when the org grants them; read tools remain |
...
- **2026-09-19 — One allowlist definition, applied three times.** `models.PerTargetToolAllowlist` is the exact tool list; dispatch restricts the run's capability snapshot to the capabilities that contribute an allowlisted tool (capped at read), the session token carries the allowlist marker plus one scope per tool and no preview, eval, or goal-improvement scope, and every internal API route is registered through a gate that refuses an allowlisted token without the route's tool scope. The in-sandbox filter applies the same list when the effective-capabilities response carries it, so an empty snapshot under an allowlist exposes nothing that needs a grant. Sentry and CircleCI reads are not in the list and stay denied. The sandbox environment also carries the list (`INTERNAL_TOOL_ALLOWLIST`), so the in-sandbox filter fails closed: a failed capability fetch or a stripped internal token leaves only tools that need no grant. Provider credentials injected for the allowlisted reads remain usable by direct API calls from the shell; that is the sandbox network boundary the design accepts, and read-only provider credentials per turn are a possible future tightening.
...
- **2026-09-20 — Completion settles the lease it completes past.** `RenewLease` matches only the fencing token, not the expiry, so a worker paused past its lease can renew it and carry on. Every completion therefore takes the job row lock first and, when the lease has expired, clears the token and records the job failed before releasing the session, the thread, and the container hold: the paused worker's next heartbeat fails and it learns it lost the job, and no job is left `running` with no lease, which nothing reclaims and which would block a drain. This is unconditional rather than reserved for lease-less recovery, because the completion fence also admits a caller holding a stale token once no live lease remains. A caller that still owns its job matches neither statement and revokes nothing. Locks are taken target, job, run, generation — the order the attempt claim (job, run) and the lifecycle transitions (target, generation) already use.
- **2026-09-20 — One lock order: target, run, job, generation.** The fenced attempt writes take the run and the job together (`FOR UPDATE OF r, j`), while the attempt claim and the first recovery helper took the job first. Either pair could hold one row and wait for the other, and an aborted attempt-end transaction loses a successful turn's result. The claim and the recovery helper now lock the run before the job, so every writer agrees.
- **2026-09-20 — Every write an attempt makes is fenced by that attempt.** A worker paused past its lease keeps running until its next heartbeat fails, so the boundary cannot rest on the worker noticing. Besides the marker, the session status, and the thread, the checkpoint publication, the turn hold release, and the terminal thread write of the failure paths now take the run row lock and apply the attempt fence: an unfenced publication could install a revoked attempt's checkpoint, an unfenced hold release could clear a later turn's hold and then destroy the container that turn is using, and an unfenced failure write could mark a thread a later turn is running on. The ownership-failure cleanup does not revert an owned session at all; its completion or the recovery that took the attempt away owns that release.
- **2026-09-20 — Every per-target thread write is fenced like the run writes.** The worker's thread write carries the agent's summary and diff, which the completion cannot reconstruct, so it stays in the handler; but unfenced it would let a worker that lost its lease overwrite the status, turn number, provider session id, summary, and diff of a turn another run has since claimed. Every thread write of a per-target turn — the continued turn's handler write, the first turn's write in the orchestrator, and the cancel path's — goes through `CompleteThreadTurnForAttempt`, which takes the run row lock and then applies the attempt fence, so an ownership change cannot commit between the fence's evaluation and the thread update. The generic failure cleanup does not write an owned session's thread at all: the completion or the recovery that took the attempt away owns that release.
- **2026-09-20 — The attempt end writes its marker before its session row.** Recovery takes target, job, run, then session; the attempt end took the session first and the run and job after, so the two could deadlock and roll back a successful turn's result. The marker write, which takes the run and job locks, now runs first inside the same transaction. Every other per-target writer, including the executing preflight, takes the target lock first.
- **2026-09-20 — The run keeps its own turn's summary.** `ListCompletedTurnSummaries` feeds the continuation prompt from `automation_runs.result_summary`, which nothing else populates for a per-target run. Completion copies it from the session, whose summary is written in the same transaction as the marker and is therefore always this turn's, and falls back to the thread only once the thread has reached this turn's number — otherwise a turn whose worker died before its thread write would inherit the previous turn's text and record it against its own head.
- **2026-09-20 — Completion is the durable thread release.** Implementation review showed a continued turn whose handler returns early leaves the primary thread `running` with stale turn metadata, so the next dispatch cannot claim it and every later run waits until timeout. The completion transaction now returns the primary thread to `idle` at the marker's own `turn_number`, which also covers a worker that died after writing the marker; the handler's richer write (summary, diff) still lands first in the ordinary path and the completion leaves an already-idle thread alone. The handler completes a successful continuation only after that write, so the target is never freed while the turn's thread is still running.
- **2026-09-20 — Completion clears a stale turn hold.** A worker that died between the result marker and its deferred `ReleaseTurnHold` leaves `turn_holding_container = true`, which would make an `awaiting_input` retirement set `ownership_release_pending` with no executing run left to clear it: the session stays automation-owned and nobody can answer it. The attempt is over by definition once its marker exists, so completion clears the hold before any retirement in the same transaction, leaving `container_id` for the next turn's inherited-container path. A per-org reconciliation also releases any ownership release left pending with no executing run, whatever stranded it.
- **2026-09-20 — Completion takes the target lock first.** Arrival, dispatch, and retirement all take the target lock before the run or the generation. Completion locked the run first, so a concurrent reset could deadlock with it and abort the user's lifecycle change. Completion and `retries_exhausted` now take the target lock through the run before touching either.
- **2026-09-20 — The sweep loop's advisory lock pins its connection.** A session-level advisory lock belongs to one connection, so taking it on a pooled connection and unlocking on whichever connection the pool hands out next leaves it held until that backend ends, and no replica ever runs the loop again. The lock holds one acquired connection for its lifetime, and destroys that connection rather than returning it to the pool when an unlock is not confirmed, so the lock dies with the backend instead of travelling to an unrelated caller; a test double without a real pool keeps the pooled statements.
- **2026-09-20 — The ambiguity deadline commits even when it converts nothing.** The candidates can leave through the per-run fallback when continuity is switched off or the kill switch fires. The transition clears `head_resolution_pending` and the deadline whenever it applies, so a stale flag cannot block every later push while the head lookup is unavailable.
- **2026-09-20 — A wake nudges the waiter's own `automation_run` job.** A waiting run's job is already polling the ownership transaction with a thirty-second backoff, so the wake handler reschedules that job to run now (or enqueues a fresh one under the same dedupe key when it is gone) instead of running the ownership transaction itself; the result is the same dispatch, without a second dispatch entry point. When no dispatchable waiter exists but a head resolution is pending, an ambiguous candidate's job is nudged so the dispatch-time lookup runs again. The wake job's handler re-runs itself when a newer request arrived while it ran. The sweeps run on their own one-minute loop under a separate advisory lock rather than on the ten-minute scheduler tick.
- **2026-09-20 — An abandoned attempt completes without a lease.** A dead-lettered job clears its lock token, so the retry path's lease-fenced completion cannot run for a marker written just before the job died. Completion is fenced by the job row: either the job is running under the caller's lease, or no live lease remains at all, in which case the attempt is over whatever happened to the worker and the marker is final. The dead-letter hook and the scheduler sweep both settle an abandoned executing run: marker present completes it, marker absent records `retries_exhausted`. A job that ended `succeeded` with an executing run and no marker is an orchestrator bug rather than an exhausted retry budget; it is still settled the same way so the owned session returns to idle and the target frees, and the summary names the missing result.
...
- **2026-09-20 — The lifecycle notification keys on the event, not on a pull request mirror.** A target exists for any pull request an automation ran on, including one 143 never created and therefore never mirrored in `pull_requests`. The notification is made from the webhook's own organization, repository, and number before any of the mirror handling, so a close reaches the targets whether or not a mirror exists; the closed transition still notifies as well, because the periodic state sync calls it when a webhook was dropped, and notifying twice for one close is a no-op. Both `PRService` instances are wired, the API's (which serves the webhook) and the worker's.
- **2026-09-20 — An unmerged close retires the generation at once.** Only a merged pull request defers retirement, and only while its subscribed merged run is still to come, because that run is the generation's last turn. An unmerged close retires immediately; `RetireGeneration` marks the ownership release pending when a turn is executing, and that turn's completion or recovery applies it. Deferring instead would strand ownership when the turn ends through a path that writes no result, and would let a reopen before completion leave the old generation active.
- **2026-09-20 — Every path that ends a target's last work retires a terminal target.** Deferring retirement for a merged final turn only works if every way that turn can end retires the generation. Marker-based completion is not the only one: the turn can dead-letter without a result, the final waiter can time out, or the turn can exit through a preflight outcome before the agent starts, which writes no marker at all. All of them call one store helper, `RetireTerminalTarget`, because an active generation left owning a session sets no pending release and the ownership sweep would never find it.
- **2026-09-20 — A reopen records its observation even when the target is already open.** Webhook deliveries can arrive out of order, and the freshness check is only as good as the evidence it compares against. A reopen that returned early on an already-open target would leave the older evidence in place, and the close it belongs with, arriving afterwards, would look fresh and retire a generation on an open pull request.
- **2026-09-20 — A lifecycle observation older than the target's own evidence is dropped.** These transitions are destructive — a close retires a generation and skips its waiters — and a webhook redelivery can arrive after a later event was applied. A close redelivered from before a reopen would otherwise retire the generation that reopen started, on a pull request that is open. An observation strictly older than `lifecycle_updated_at` is dropped before any side effect; an observation the target has no evidence for is accepted.
- **2026-09-20 — Completion retires only when nothing is left to run.** A turn completing on a terminal target retires its generation, but not while another run is still waiting or executing on that target: otherwise the turn ahead of a merged final turn would retire the generation the final turn is meant to finish in.
- **2026-09-20 — The lifecycle notification is best-effort, and completion is the second chance.** It runs outside the transaction that records the pull request's own state, so a failure there cannot fail the webhook. What it would have done is bounded anyway: the wait timeout and the recovery sweep still end the waiters, and a completion on a target whose lifecycle is already terminal retires the generation itself. A redelivered close changes nothing the first one did not.
