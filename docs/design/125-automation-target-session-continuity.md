...
- **2026-09-20 — Recovery revokes the lease it recovers from.** `RenewLease` matches only the fencing token, not the expiry, so a worker paused past its lease can renew it and carry on. Recovery therefore takes the job row lock and clears the token of a job whose lease has expired before it releases the session, the thread, and the container hold; the paused worker's next heartbeat fails and it learns it lost the job. Locks are taken target, job, run, generation — the order the attempt claim (job, run) and the lifecycle transitions (target, generation) already use.
- **2026-09-20 — The turn's thread write is fenced like every other attempt write.** The worker's thread write carries the agent's summary and diff, which the completion cannot reconstruct, so it stays in the handler; but unfenced it would let a worker that lost its lease overwrite the status, turn number, provider session id, summary, and diff of a turn another run has since claimed. A per-target turn writes the thread through `CompleteThreadTurnForAttempt`, under the same attempt fence as the run writes.
- **2026-09-20 — The run keeps the turn's summary.** `ListCompletedTurnSummaries` feeds the continuation prompt from `automation_runs.result_summary`, which nothing else populates for a per-target run. Completion copies the turn's summary from the thread, or from the session for a fresh generation's first turn; both are durable before completion runs, so a recovery records the same text.
