...
- **2026-09-20 — A terminal job with a marker completes without a lease.** A dead-lettered job clears its lock token, so the retry path's lease-fenced completion cannot run for a marker written just before the job died. Completion is fenced by the job row: either the job is running under the caller's lease, or the job is terminal, in which case no attempt can be live and the marker is final. The dead-letter hook and the scheduler sweep both settle an executing run whose job is terminal: marker present completes it, marker absent records `retries_exhausted`. A job that ended `succeeded` with an executing run and no marker is an orchestrator bug rather than an exhausted retry budget; it is still settled as `retries_exhausted` so the owned session returns to idle and the target frees, and the summary names the missing result.
