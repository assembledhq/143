...
- Routine worker deploys keep fleet rollouts all-or-nothing by default. The blue/green capacity preflight blocks on free memory, while idle CPU is observed but non-blocking unless an operator explicitly configures `WORKER_BLUE_GREEN_MIN_IDLE_CPU_MILLIS`.
