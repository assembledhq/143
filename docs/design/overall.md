...
- Step 3: Execute a coding agent
    ...
    - Attached session review comments are **single-use prompt context**. When the user submits a follow-up turn, those attached comment chips and the draft input should clear immediately from the composer UI, the review block should be injected into that one outbound message, and later follow-up turns should not silently replay the same comments unless the user explicitly reattaches or reopens them.
    ...
    - Session detail should also support an explicit **agent self-review** action that asks the configured coding agent to review the current diff using native review affordances where available, but it must be clearly positioned as self-review rather than independent approval and must only surface when the session has a resumable diff. Claude Code routes review turns through `/review` and `/security-review`; Codex routes them through native `/review`. Detailed design lives in [implemented/63-agent-native-review-button.md](implemented/63-agent-native-review-button.md).
