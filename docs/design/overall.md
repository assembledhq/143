...
- Direct session-detail opens should keep the route entry module thin enough to paint the normal session skeleton before the full chat/review/preview client surface loads. The authenticated shell may prefetch session detail data in parallel with auth, while the heavy detail module remains split so cold URL opens are not blocked on evaluating every session sub-surface before first paint.
