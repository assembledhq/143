...
- Step 3: Execute a coding agent
    ...
    - Multi-node deployments should use shared object storage for session snapshots rather than node-local disk so worker-written snapshots can be hydrated later by API nodes; see [future/54-s3-session-snapshots.md](future/54-s3-session-snapshots.md).
