...
- GitHub PRs generated by 143 should act as a durable discovery surface back into the originating session. Every generated PR should include an explicit link to the producing session using the deployment's configured app base URL, but that link must still respect normal 143 authentication and org-membership checks rather than becoming a public transcript bypass. Detailed design: [future/67-pr-session-links-and-viewer-access.md](future/67-pr-session-links-and-viewer-access.md).
