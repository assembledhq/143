...
- When Linear is actively integrated, the manual-session composer should expose an `Add linear issue` affordance in the add menu and render added issues as removable chips in the composer instead of dumping pasted Linear refs back into the raw textarea. The action should stay hidden when Linear is not connected.
    ...
    - The session conversation view should use **progressive disclosure for agent tabs**: before a user creates a second tab, render only a quiet active-agent header with a local `Add tab` affordance rather than the full tab strip. Once a second tab exists, expand that same slot into the full strip. Detailed design: [future/70-session-tab-strip-progressive-disclosure.md](future/70-session-tab-strip-progressive-disclosure.md).
