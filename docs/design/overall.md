...
- Connected preview secrets use preview secret bundles managed from Settings → Preview. Repo config may reference a bundle by name, allowlist env vars, and target services; runtime launch decrypts the bundle for the active org/repository and passes values as process environment variables rather than shell-interpolated command text. The Preview settings page is the single admin surface for both repo-scoped preview secret bundles and scoped preview API tokens, using table-based inventories with dialogs/sheets for create/edit flows; see [implemented/88-preview-settings-page.md](implemented/88-preview-settings-page.md).
