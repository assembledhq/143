...
- Step 3: Execute a coding agent
    ...
    - Automations should be able to run an optional **pre-PR self-review loop** before publication: a simple automation setting chooses `0` to `5` review/fix passes, the coding agent runs its standard review command against its own diff, fixes issues in the same sandbox, stops early when the agent reports the review is clean, and only proceeds to PR creation when the loop exits clean or policy allows bypass. Detailed design: [future/78-review-agent-loops.md](future/78-review-agent-loops.md).
...
- When Linear is actively integrated, the manual-session composer should expose an `Add linear issue` affordance in the add menu and render added issues as removable chips in the composer instead of dumping pasted Linear refs back into the raw textarea. The action should stay hidden when Linear is not connected.
    ...
    - Session detail should expose a **Review** action that starts a bounded review/fix loop inside the current sandbox: create one review-loop tab, run the selected agent's native `/review` command against the live working tree, let that same agent fix its own findings, and repeat sequentially until the agent reports clean or the pass limit is reached. The UI must make clear that the loop runs in the session's agent container and produces one combined branch/PR. Detailed design: [future/78-review-agent-loops.md](future/78-review-agent-loops.md).
...
- Reopening an existing session should optimize for continuation rather than chronology: restore the user's last reading position when available; otherwise open active sessions at the live edge and inactive sessions at the start of the latest assistant turn rather than the top of the transcript. In multi-thread sessions, the continuation state also includes the last active thread; reopen should restore that thread before applying the per-thread scroll anchor so the UI does not silently fall back to the first tab. Legacy pre-migration scroll keys that only recorded a raw numeric `0` should be treated as "no meaningful saved position" so older top-of-thread state does not mask the newer continuation-first reopen behavior; structured saved positions remain authoritative, including an intentional top position. Mount-time skeleton state is not a meaningful saved position and must not be persisted before the initial anchor decision resolves. Detailed rationale lives in [future/56-session-open-position.md](future/56-session-open-position.md).
    ...
    - Production worker hosts treat the sandbox-auth socket directory as a startup invariant. First-boot cloud-init, provisioning, and routine worker deploys all create `/var/run/143/sandbox-auth` as uid/gid `1000:1000` with mode `0750`; production workers preflight the directory before registering as job-capable so a Docker-created `root:root 0755` bind-mount source cannot claim sessions and fail later at turn start.
