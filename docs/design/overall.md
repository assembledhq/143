...
- Step 3: Execute a coding agent
    ...
    - Manual-session cancel should interrupt the **actual agent CLI process**, not just the worker-side stream reader. Each agent launch writes its live PID inside the sandbox before `exec`-ing the CLI, and cancel targets that tracked process through a per-agent cancellation spec. The platform defaults new agents to **Ctrl+C / SIGINT** unless the adapter declares another method, so the stop path stays explicit as the agent roster grows.
