...
- Step 3: Execute a coding agent
    ...
    - PR creation failures should use a short toast (`PR creation failed`) and keep the actionable failure detail inline beside the `Create PR` / `Retry` control in the session header, so the cause stays attached to the PR action even after the toast disappears.
