...
- Free-form form fields that can receive focus on phones, especially multi-line composers, automation inputs, and settings textareas, must compute to at least `16px` on small screens to prevent iOS Safari auto-zoom while preserving denser desktop typography. Automation select-style controls should match the same mobile/desktop text sizing and height as neighboring text inputs.
...
- Preview readiness is a two-sided contract: the application must pass its in-sandbox readiness probe, and the worker must be able to dial the sandbox container IP/primary port before the preview is marked ready. Resource sizing is tiered by topology so single-service previews stay cheap, normal multi-service previews remain modest, and multi-service previews with managed infrastructure get enough CPU/memory to avoid starving their own dev servers without exceeding the normal sandbox slot model.
- Preview startup failures should keep diagnostics in one inline surface: the compact failure summary expands in place into the full persisted startup logs, avoiding nested cards or separate drilldown panes for the common "what failed and what should I retry" flow.
- Preview lifetime controls should keep the default posture short-lived while giving power users bounded adjustment from session detail: a hidden Preview-tab lifetime menu for extend/shorten actions plus an imminent-shutdown banner that appears only near expiry. Detailed design: [implemented/82-preview-lifetime-controls.md](implemented/82-preview-lifetime-controls.md).
...
- PR Overview commands should preserve the original PR session as the single transcript. Agent-backed commands such as `Fix tests` and `Resolve conflicts` should continue that session with short visible prompts and structured command context, using normal snapshot continuation when trustworthy and PR-head reconstruction when needed; direct commands such as `Push changes` and `Merge` stay session-bound without creating repair sessions. Detailed design: [implemented/76-pr-repair-session-continuity.md](implemented/76-pr-repair-session-continuity.md).
...
- Step 1: Ingest and aggregate customer and engineering context from:
    ...
    - Linear can also trigger coding-agent sessions directly when the 143 app user is assigned or @-mentioned. Signed `AgentSessionEvent` webhooks create a `linear_agent_sessions` bridge row, enqueue `linear_agent_event`, resolve the target repository from team/project/label/default mappings, and then run the normal session workflow. The Linear integration settings surface exposes the org default repository fallback plus team/project-specific repository overrides so admins can use a best-effort default while routing individual Linear teams to different repos. Linear-created 143 sessions use the same session-level autonomy/token defaults as other automatic session creation paths, and pre-session failures posted back to Linear include a direct operator debug link for the Linear AgentSession bridge row. The process-wide `LINEAR_AGENT_ENABLED` flag gates new inbound webhooks only; queued jobs and already-created sessions continue to drain.
...
- Reopening an existing session should optimize for continuation rather than chronology: restore the user's last reading position when available; otherwise open active sessions at the live edge and inactive sessions at the start of the latest assistant turn rather than the top of the transcript. The preferred implementation loads the newest useful message window first, uses a minimal bottom-aligned transcript skeleton only until that first real window is ready, and fetches older history above the viewport without moving the content the user is reading. In multi-thread sessions, the continuation state also includes the last active thread; reopen should restore that thread before applying the per-thread scroll anchor so the UI does not silently fall back to the first tab. Legacy pre-migration scroll keys that only recorded a raw numeric `0` should be treated as "no meaningful saved position" so older top-of-thread state does not mask the newer continuation-first reopen behavior; structured saved positions remain authoritative, including an intentional top position. Mount-time skeleton state is not a meaningful saved position and must not be persisted before the initial anchor decision resolves. Detailed rationale lives in [implemented/56-session-open-position.md](implemented/56-session-open-position.md).
    ...
    - The sandbox is the filesystem permission boundary for coding agents. Agent CLIs launched inside the gVisor-isolated container should auto-approve routine in-workdir execution when their auth tier, CLI version, and model support it so users do not have to grant file-by-file or `git diff`-style approvals after choosing to run the agent. Claude Code uses `auto` permission mode only for compatible credential, CLI-version, and model-family combinations and falls back to `acceptEdits` rather than full bypass mode, while 143 only intercepts explicit human questions. Network-capable actions still need explicit sandbox or tool-level policy because production sandboxes intentionally retain public internet egress while blocking metadata and private network destinations.
    ...
    - Worker capacity is visible from the primary Grafana operations dashboard. Workers emit low-volume platform health samples for current running sessions, turn-held sandboxes, active previews, preview-held sandboxes, running jobs, and host CPU/RAM utilization by worker node so operators can compare current and historical load when deciding whether to scale the worker fleet.
