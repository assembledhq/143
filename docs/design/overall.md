...
- Step 3: Execute a coding agent
    ...
    - Future session detail should support **multiple agent tabs inside one sandbox**, with the tab strip directly above the transcript and shared composer rather than in the session-details side panel. A session owns the sandbox and final PR, while each tab owns its own transcript, agent process, status, cost, checkpoints, and coordination signals. (In the UI we call these tabs; in backend code they are threads.) Use this only when agents should share the same branch and filesystem; independent PR streams should remain separate sessions. Detailed design: [future/68-sandbox-agent-tabs-and-threads.md](future/68-sandbox-agent-tabs-and-threads.md).
