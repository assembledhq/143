...
- Private Connector is the outbound-only path for customer private-network resources. The implemented foundation covers connector records, deployment-token bootstrap and policy enforcement, durable connector identity, signed heartbeats/actions/config pushes, WebSocket gateway sessions, VictoriaLogs provider routing and policy enforcement, read-only Postgres query/schema/explain/index/sample-row routing, database MCP tools, resource health tests, lifecycle controls, health alert webhooks, UI-triggered connector update actions, preview DB lease records/token authorization with repository and active-lease policy, and the Settings -> Integrations setup surface; provider subprocess isolation, scoped preview DB proxying/env injection, secondary gRPC/mTLS transport, and health-weighted routing remain staged follow-up work. See [future/108-private-connector.md](future/108-private-connector.md).
...
| Slack collaboration | Signed callbacks, durable webhook ingress, Slack-started sessions, App Home, per-user org retargeting, human-input delivery, notification fanout, preview/PR controls | [future/92-slackbot-product-surface.md](future/92-slackbot-product-surface.md), [implemented/101-slackbot-implementation-plan.md](implemented/101-slackbot-implementation-plan.md), [implemented/100-slack-webhook-ingress-durability.md](implemented/100-slack-webhook-ingress-durability.md) |
...
| Automations and external entry points | Team-owned scheduled/event automations, capability policies, goal-first creation, goal-improvement proposals, review loops, Slack actions, external API, API-key settings, CLI-triggered platform workflows | [implemented/31-automations-tab.md](implemented/31-automations-tab.md), [implemented/48-automations-separation.md](implemented/48-automations-separation.md), [implemented/84-automation-goal-first-ux.md](implemented/84-automation-goal-first-ux.md), [implemented/94-external-api-sessions-automations.md](implemented/94-external-api-sessions-automations.md), [implemented/100-api-key-settings-and-external-api-docs.md](implemented/100-api-key-settings-and-external-api-docs.md), [implemented/95-hierarchical-agent-tools-cli.md](implemented/95-hierarchical-agent-tools-cli.md), [implemented/101-slackbot-implementation-plan.md](implemented/101-slackbot-implementation-plan.md), [implemented/103-agent-capabilities.md](implemented/103-agent-capabilities.md), [future/105-automation-goal-improvement.md](future/105-automation-goal-improvement.md) |
| Private infrastructure access | Outbound-only connector records, one-time deployment-token bootstrap with policy enforcement, durable Ed25519 connector identity, signed heartbeats/actions/config pushes, WebSocket gateway transport, VictoriaLogs and read-only Postgres action providers/tools, preview DB lease records/token authorization with repository/active-lease policy, resource health tests, lifecycle controls, health webhooks, UI-triggered update actions, and settings UI; per-provider subprocess isolation, scoped preview DB proxy/env injection, secondary gRPC/mTLS transport, and health-weighted routing remain future phases | [future/108-private-connector.md](future/108-private-connector.md) |
