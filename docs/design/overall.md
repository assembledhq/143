...
- Step 3: Execute a coding agent
    ...
    - Automations may also subscribe to GitHub repository events in addition to their schedule. The supported event triggers are PR created, PR conversation comment added, PR review completed, and inline PR review comment added. GitHub-triggered runs reuse the normal `automation_run` worker path with `triggered_by = github` and append concise event context to the run goal snapshot.
