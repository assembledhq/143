...
- Automation creation and detail are now goal-first surfaces: `/automations/new` opens on a centered composer with inline execution defaults, compact template starters, and advanced settings in a sheet; `/automations/:id` leads with readable goal markdown, latest-run context, full history, and a supporting metadata rail. Detailed design: [implemented/84-automation-goal-first-ux.md](implemented/84-automation-goal-first-ux.md).
