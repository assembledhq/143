...
- Step 4: Open PR and ship
    ...
    - PR description generation should balance the code diff, files changed, issue/session title, and summaries from all visible session threads. The latest session or review-loop summary is only one signal; it should not cause the PR description to become a review report instead of a reviewer-facing explanation of the code change.
