...
- Browser-tab favicons use the original aircraft mark on a transparent background, cropped and simplified only enough to stay legible at 16px. Favicon exports should preserve the aircraft silhouette, blue body gradient, cockpit, nose, and highlight details while avoiding badge backgrounds, drop shadows, dark side-cockpit slivers, or black exterior borders that can read as browser chrome artifacts. Small ICO entries should be rendered directly at their target size with a subtle light silhouette stroke behind the gradient so the plane reads as one connected mark instead of sparse pixels.
...
- Existing session detail opens at the most relevant continuation point instead of the top: saved per-user/per-thread position wins, active threads land at the live edge, inactive threads anchor to the latest assistant turn, and multi-thread sessions restore the last active thread before transcript content is shown. Long thread restores persist message anchors plus offsets and reopen with a bounded `position=around` message window, with older/newer pagination around that anchor and a latest-window bypass for `Jump to latest`. Session-list clicks should switch routes immediately using native links plus provisional detail cache seeding, show the normal detail skeleton while authoritative detail loads, and only start session-specific child requests after the real detail payload arrives. Thread messages also expose cursor-friendly latest, older, newer, and anchor-window APIs with anchor metadata for bounded loading; see [implemented/56-session-open-position.md](implemented/56-session-open-position.md).
...
- Step 4: Open PR and ship
    ...
    - Session detail now includes a compact **PR health** row near the existing top-of-Overview PR/error notice area, backed by synced GitHub state, reconciliation, and org-scoped SSE updates so operators can see conflicts or failing tests quickly and launch one-click repair actions like `Resolve conflicts` and `Fix tests`. These repair actions continue the canonical PR session and, when launched from a multi-thread session view, run in the currently active thread/tab rather than silently routing to the first-created Main thread; see [implemented/61-pr-state-sync-and-repair-actions.md](implemented/61-pr-state-sync-and-repair-actions.md) and [implemented/76-pr-repair-session-continuity.md](implemented/76-pr-repair-session-continuity.md).
