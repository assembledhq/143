...
- Step 2: Prioritize and identify top issues based on business impact
    ...
    - Session and project sidebars should feel native on phones: a left swipe reveals an archive action beneath the row, with an iOS-style icon-led affordance, progressive fill/ready-state feedback as the user approaches the auto-archive threshold, tap-to-close behavior when the row is already open, fully opaque resting rows so the hidden action tray never bleeds through, and a centered single-surface archive tray instead of layered sub-panels.
- Step 3: Execute a coding agent
    ...
    - On desktop, the session composer can expose repo, branch, model, and reasoning controls inline. On mobile, it now switches to a **launch-first layout** that keeps the textarea and submit action visible while moving advanced run settings into a compact secondary surface across new-session, quick-create, and resumed follow-up composer flows.
    ...
    - On mobile, diff review should use a **dedicated full-screen file reader** rather than relying on the desktop-style split between the center review pane and the detail-sheet file list. The conversation-level `Files changed` affordance should jump straight into that diff reader, while the bottom-sheet `Changes` panel remains the file index for subsequent navigation.
    - The grouped `Changes` file tree should preserve the parsed diff's **first-seen sibling order** instead of applying a separate directory-first sort, so the sidebar and diff viewer stay aligned whenever the tree structure can represent the same sequence. Exact identity is not always possible when a flat diff order interleaves files from the same directory with files elsewhere; see [implemented/68-consistent-review-file-ordering.md](implemented/68-consistent-review-file-ordering.md).
...
4. **Worker sandbox concurrency model** — ✅ Worker parallelism is now configurable per node via `WORKER_PROCESS_COUNT` (default `1`), so each server can run multiple in-process worker loops and therefore multiple concurrent sandbox-backed jobs. Sandbox sizing is configurable via `SANDBOX_CPU_LIMIT`, `SANDBOX_MEMORY_LIMIT_MB`, and `SANDBOX_DISK_LIMIT_GB`; org-level run concurrency remains enforced separately by `max_concurrent_runs` (default 10). Self-hosting sizing guidance lives in `docs/self-hosting/worker-capacity-tuning.md`.
