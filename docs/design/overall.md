...
- Expired invitations must be explicit on admin team surfaces. Invitees only see claimable, unexpired invitations, while admins see unresolved expired rows labeled as expired with guidance to revoke and resend instead of a misleading generic pending invite.
