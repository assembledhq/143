...
- Step 3: Execute a coding agent
    ...
    - Automations support an optional **reasoning-level override** alongside model selection. The value is validated, rendered, and capability-gated through the same agent-specific reasoning helpers used by sessions, then persisted on the automation, copied to spawned sessions, and captured in automation run config snapshots so future reasoning-tier changes stay centralized.
