...
- The sandbox image installs the supported coding-agent CLIs, including Codex, Claude Code, OpenCode, Amp, and Pi. Runtime credentials are resolved from ordered personal/team auth stacks with health and rate-limit state tracked separately from credential config. OpenCode defaults to GLM 5.2; open-source model setup uses audited US-based OpenCode or OpenRouter inference routes, with OpenRouter GLM constrained in generated config to audited provider slugs and no fallback to unaudited providers.
