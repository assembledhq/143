...
- Step 1: Ingest and aggregate customer and engineering context from:
    ...
    - Linear can also trigger coding-agent sessions directly when the 143 app user is assigned or @-mentioned. Signed `AgentSessionEvent` webhooks create a `linear_agent_sessions` bridge row, enqueue `linear_agent_event`, resolve the target repository from team/project/label/default mappings, and then run the normal session workflow. The process-wide `LINEAR_AGENT_ENABLED` flag gates new inbound webhooks only; queued jobs and already-created sessions continue to drain.
