...
- Step 3: Execute a coding agent
    ...
    - Reopening an existing session should optimize for continuation rather than chronology: restore the user's last reading position when available; otherwise open active sessions at the live edge and inactive sessions at the start of the latest assistant turn rather than the top of the transcript. Detailed rationale lives in [future/56-session-open-position.md](future/56-session-open-position.md).
    - Session detail titles support **selection-safe inline editing**. The header keeps the title as normal selectable text, reveals a lightweight edit affordance on hover/focus, and only enters edit mode on an intentional click so drag-selection and double-click word selection do not accidentally trigger renaming.
