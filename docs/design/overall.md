...
- GitHub organization auto-join lets admins capture a linked GitHub App organization as an auto-join source. New and existing GitHub OAuth users are granted memberships after live GitHub org-membership confirmation, rosters are maintained through webhooks plus durable reconciliation jobs, and Team/Integrations settings expose the policy and its operational warnings. See [implemented/98-github-org-auto-join.md](implemented/98-github-org-auto-join.md).
...
- The official external API uses org-scoped API clients as durable service-account principals and one-time-shown `143_sk_` bearer tokens as disposable credentials. Tokens carry explicit resource/action scopes, optional repository allowlists, last-used metadata, revocation/expiry state, and idempotency support for retryable session and automation mutations. Browser cookie routes and bearer-token external calls share the `/api/v1` contract, but API-token requests resolve to API-client context rather than human roles and are scope-checked before reaching session, automation, preview, or future public resources. Detailed design: [implemented/94-external-api-sessions-automations.md](implemented/94-external-api-sessions-automations.md).
...
- Form-oriented modals should use the shared responsive modal pattern: centered constrained dialogs on desktop, bottom sheets on phones, fixed header/footer regions, and a scrollable body so the primary action remains reachable on short mobile viewports and when the keyboard is open. Auth setup flows, including coding-agent auth, Codex device code, and Claude subscription auth, should use this shared primitive instead of hand-rolled fixed overlays.
...
- Frontend server-state should prefer a short local-freshness window plus explicit invalidation for volatile workflows over immediate staleness everywhere. Repeated navigation should render from React Query cache first, reconcile through SSE/polling/mutation invalidations, and optimistically update narrow caches for common row-level mutations such as session archive/unarchive while preserving rollback on failure. Hot navigation paths such as session rows should prefetch their likely destination on hover or keyboard focus.
...
- Global app navigation should use a compact desktop type scale and a touch-friendly mobile drawer scale. Desktop sidebar labels remain dense at `text-xs`, compact desktop uses icon-only rail navigation, and mobile drawer rows use `text-sm` labels with larger tap targets for both primary and nested settings navigation.
...
- Preview controls should treat launch, retry, and restart as one user intent to get the session running with the current workspace config, keep startup diagnostics inline, and keep lifetime adjustment bounded behind secondary actions. The preview detail route is the canonical status/control surface; `?launch=1` only adds auto-start/auto-open intent on that same page rather than rendering a separate interstitial. Detailed designs: [implemented/82-preview-lifetime-controls.md](implemented/82-preview-lifetime-controls.md), [implemented/84-preview-command-header.md](implemented/84-preview-command-header.md).
- Session preview freshness is backend-authoritative: sessions carry a monotonically increasing workspace revision, preview launches/recycles stamp the revision they started from, HMR/file events can stamp the runtime-observed revision, and the Preview tab distinguishes quiet live updates from refreshable staleness and restart-required changes such as dependency, preview config, build config, environment config, or schema edits. Detailed design: [implemented/89-session-preview-freshness.md](implemented/89-session-preview-freshness.md).
...
- The `/previews` index is org-scoped and target-oriented for branch/PR previews, but it must also surface active and recent session previews that still have no `preview_target_id`. These runtime-only rows use the runtime preview ID as the list/detail ID until they are attached to a durable branch preview target.
...
- Slack is also a first-class product surface, not only a context-ingestion source. Signed Slack Events, slash-command, and interaction callbacks resolve the active Slack installation, dedupe inbound events, and enqueue durable jobs. Mentions and bot DMs create or continue normal 143 sessions with Slack thread metadata persisted in `slack_session_links` and sanitized source metadata in `session_attributions`; Slack remains a lightweight thread surface while the canonical transcript, diffs, previews, PR state, and logs stay in 143. Slack notifications cover session, automation, PR, preview, and human-input event families, including automation failure streaks and stale previews, and honor channel DM visibility for general notification fanout. Slack App Home aggregates pending responses, Slack-linked session previews, subscribed automation runs, and an actionable multi-org selector for mapped users. Slack-origin actions share authorization for channel capabilities, mapped-user roles, and originating team-session allowances across session starts, preview controls, PR repair/merge requests, and human-input answers. The backend surface is partially implemented; the detailed design tracks completed and remaining work in [future/92-slackbot-product-surface.md](future/92-slackbot-product-surface.md).
...
- Admins can manually kick off a new linked session from an Autopilot queue row even when the automation state is blocked, failed, or skipped, as long as the issue has a repository. The kickoff sheet accepts optional session notes, stored as the initial user message so the agent receives the operator's extra context while the session remains attached to the original issue.
...
- When Linear is actively integrated, the manual-session composer should expose an `Add linear issue` affordance in the add menu and render added issues as removable chips in the composer instead of dumping pasted Linear refs back into the raw textarea. The action should stay hidden when Linear is not connected.
    ...
    - Follow-up workspace mention search should stay fast on large repos by caching the session path index against the actual workspace source fingerprint, avoiding activity-time invalidation, and excluding dependency/build output directories from the mention index.
    ...
    - The sessions sidebar should derive its active-row highlight from the active child route under the `/sessions` layout, not from assumptions about leaf-route params being available in shared layout UI. If the current session appears in the list, it should remain highlighted across client navigation, while `/sessions/new` must never highlight a historical session row. When the currently open session is not present in the current sidebar result set because of filters, search, pagination, or direct navigation, the sidebar should show a selected contextual current-session row in the same top slot used by the new-session draft, separated from saved-session results, instead of selecting the first visible saved session. The active state should read clearly at a glance through a slightly stronger tinted surface and edge treatment, but stay restrained enough that the sidebar still feels calm and consistent.
    ...
    - The long-term product model for session tabs is **tmux panes in one shared sandbox**: the session owns the sandbox, worktree, branch, preview, PR, and final shipping artifact, while each tab/thread owns an independent agent runtime, durable inbox, transcript, status, cancellation path, and delivery cursor. Backend admission now allows multiple admitted sibling tabs to process concurrently inside the same session-owned sandbox, with explicit sandbox holders, lease-fenced thread runtimes, thread-deduped continuation jobs, retry-on-sibling-publish behavior for shared-container races, grouped inbox delivery summaries on thread and session-detail payloads, adapter-declared provider-native input protocols, dead-letter/unknown-delivery replay UX, snapshot quiescence checks for publish paths, and reaper recovery for expired runtime leases. Executor recovery must also treat a valid active thread-runtime lease as authoritative: retrying the same thread waits for runtime lease recovery instead of starting a duplicate runtime, and uniqueness conflicts are classified as transient runtime ownership rather than user-visible failures. Detailed design: [implemented/88-shared-sandbox-thread-runtimes.md](implemented/88-shared-sandbox-thread-runtimes.md).
...
- The session review surface is a structured code-review UI rather than a raw diff blob: parsed file/hunk rendering, inline comments, keyboard navigation, repo browsing, and scroll-synced file-tree highlighting are part of the intended core loop. Review now supports **GitHub-style directional context expansion** with boundary-specific icon controls anchored in the line-number gutter: up arrows above changes reveal prior context, and down arrows below changes reveal following context. This is backed by richer file-context metadata from the sandbox. The diff collection path also has a dedicated authoritative collector that compares the workspace against the immutable recorded base commit and appends untracked files, with typed snapshot persistence for review provenance; see [implemented/55-code-diff-context-navigation.md](implemented/55-code-diff-context-navigation.md). PR creation continues to use snapshot-backed workspace pushes for shipping fidelity.
    ...
    - Production sandboxes share the `143-sandbox` bridge with preview infrastructure and a `sandbox-dns` sidecar at pinned bridge IPs for both the default and static-egress sandbox networks. The bridge must leave Docker inter-container communication at its default so gVisor sandboxes can reach that DNS sidecar; host firewall rules provide the egress boundary for metadata and private-network destinations.
    ...
    - Production worker hosts use a single idempotent reconciliation script (`deploy/scripts/reconcile-worker-host.sh`) for host invariants shared by cloud-init, SSH provisioning, routine deploys, and manual repair. It owns the pinned `143-sandbox` bridge, sandbox egress firewall, `/etc/143/sandbox-resolv.conf`, worker sysctl, and `/var/run/143/sandbox-auth` as uid/gid `1000:1000` with mode `0750`. Static egress reconciliation starts the `sandbox-dns` sidecar before probing the static-egress bridge, and worker reprovision drains blue/green worker generations before recreating pinned bridge services so old workers cannot occupy the DNS IP. Both worker cloud-init and SSH provisioning install the same narrow deploy-user `NOPASSWD` sudo grant so first-boot workers can be added to `FLEET_HOSTS` and survive routine fleet deploys without requiring root SSH repair. Production workers still preflight the sandbox-auth directory before registering as job-capable so a Docker-created `root:root 0755` bind-mount source cannot claim sessions and fail later at turn start.
    ...
    - Long-running sessions must survive routine platform deploys and worker restarts. Worker infrastructure therefore uses deploy guardrails, lease-based dead-owner recovery for in-flight jobs, DB-owned running-job verification before worker shutdown completes, checkpoint-aware resume from the last committed turn after unplanned owner loss, and durable per-session executors that move `run_agent`/`continue_session` turn ownership out of the deployable worker process. Once a sandbox-bound retry discovers that a live container is owned by a specific worker, the retry must persist `target_node_id` so the next attempt goes back to that worker; if that worker is dead, the existing dead-node fallback may claim the job elsewhere and clear the stale container reference before hydrating from snapshot. `run_agent` recovery follows the same wrong-node targeting path as follow-up turns so checkpointed recovery does not get trapped on an unpinned retry after a worker generation is replaced. When a user sends queued input to a busy thread, the platform also performs targeted owner-loss recovery for that session before the periodic recovery loop fires, while keeping the message durable and queued for checkpoint restore. Recovery is bounded when no durable checkpoint exists: after repeated worker-loss attempts before the first checkpoint, the session/thread fail with recovery-specific metadata instead of restarting the same turn indefinitely. Worker drain uses typed `worker_drain` stop requests; the executor requeues the owned job after the orchestrator restores a recoverable status and, when possible, stores an interruption checkpoint without advancing the turn. Recovery logs must include whether a checkpoint was available and whether the platform restarted from scratch so lost intra-turn work is visible. Sandbox-capacity retry exhaustion and stale-sandbox cleanup dead-letter exhaustion likewise terminalize the affected session/thread so the UI does not remain stuck in `running`; the user-facing failure copy stays concise while final slot counts and worker-load details are logged for operators. See [51-worker-deploy-safety.md](backlog/51-worker-deploy-safety.md) and [82-durable-session-executors.md](implemented/82-durable-session-executors.md).
    - The deploy contract is stricter than the old worker-drain recovery path: routine blue/green worker deploys move new work to the green generation without sending stop signals to active session executors or active preview runtimes. Old generations stay routable for owned work until turns complete, ask for human input, hit normal runtime policy, or exhaust explicit preview/runtime ownership policy. Routine deploys require a reachable worker blue/green host-port range; Make defaults this to `8080-8087`, so the default operator shape is `make deploy-fleet ROLES=app,worker`. Fleet deploys run matching entries concurrently with bounded fan-out (`DEPLOY_JOBS`, default 4) while serializing role deploys that target the same host, and keep per-host logs under `/tmp`; operators can set `DEPLOY_JOBS=1` for a fully serialized rollout. Routine deploys fail closed when no safe endpoint is available. Maintenance deploys that restart Docker, support services, host networking, or the host itself are separate, intentionally disruptive operations that block by default while active runtimes exist. Worker deploy fingerprints are classified by blast radius: worker-process config can roll through routine blue/green, while support-service, host-runtime, and Docker-daemon/runtime config changes remain maintenance-gated. The implementation foundation and long-term controller requirements are in [future/91-non-disruptive-worker-blue-green-deploys.md](future/91-non-disruptive-worker-blue-green-deploys.md).
    - Worker decommissioning has a dedicated spin-down path that drains worker generations and durable session executors before stopping compose-managed worker services. Destructive machine cleanup is an explicit `CLEAR=true` operator action that removes leftover executor/sandbox containers and unused Docker state without deleting host provisioning files. See [implemented/90-worker-spin-down-ops.md](implemented/90-worker-spin-down-ops.md).
    - Session runtime control should use a **soft default budget plus graceful, checkpoint-first shutdown**, not a single blunt timeout. Healthy long-running work may extend within policy; when the platform must stop a run it should first try to preserve resumable state and surface clearly whether the latest turn was checkpointed or the user is resuming from an earlier committed state. The implemented control plane now tracks per-session soft/hard deadlines, a 15-minute no-progress budget, parsed active tool execution, recent progress signals, bounded runtime extensions, graceful-stop reasons, checkpoint metadata, and queued/recovering worker-recovery state so runtime decisions and resume UX are observable in the session record. Runtime start persists `running`, `started_at`, failure cleanup, and deadline initialization atomically; watchdog scans ignore soft deadlines that predate the current start; and continue-session startup cleanup uses detached bounded contexts so request cancellation does not leave stale `running`/sandbox state behind. Adapter-level CLI liveness is not progress by itself: only parsed tool/sandbox lifecycle events suppress no-progress shutdown. Runtime watchdog failures terminalize matching `run_agent`/`continue_session` jobs, and lease renewal refuses to keep session jobs alive after their session is terminal. Detailed runtime behavior lives in [future/60-agent-runtime-timeouts-and-checkpointed-shutdown.md](future/60-agent-runtime-timeouts-and-checkpointed-shutdown.md).
    - Optional live preview for sandbox output is served from an **isolated preview origin** using short-lived preview tokens, never from the main app origin. This prevents untrusted preview code from sharing the app's authenticated browser context. In multi-node production the public preview edge stays on app nodes while sandbox hydrate/reuse, preview lifecycle, inspector actions, and cleanup run on worker nodes. Initial preview startup is a durable, idempotent `start_preview` worker job so app deploys, worker replacement, and client disconnects do not cancel or corrupt hydrate/build/readiness. Preview target and preview instance are durable user-facing identities; preview runtime is the live leased worker attachment that owns the sandbox/server endpoint. Routing uses `preview_runtimes.endpoint_url`, and gateway tokens bind to runtime ID plus epoch so old tokens cannot target a different worker runtime. App-to-worker preview RPC uses the ordered `PREVIEW_RPC_SECRETS` keyring, signing with the first configured secret and validating against all configured secrets; when unset, it falls back to `SESSION_SECRET` for rollout compatibility. Routine app deploys run a fleet-wide internal `auth_check` probe against active/draining workers that advertise `preview_rpc_auth_check`; routine worker deploys run the same probe only against the newly started candidate worker before draining older local generations. Incompatible preview RPC secrets fail closed without blocking the first rollout that introduces the probe endpoint, and worker deploys do not require worker-to-worker reachability across the whole fleet. Worker deploys use blue/green worker generations: the new worker starts with a unique node ID and host port, passes Docker health and DB-heartbeat readiness, becomes eligible for new work, then old workers are marked draining in the background. Draining workers are excluded from new cold-start selection but remain routable for previews they already own until those previews stop or the preview drain timeout expires. If an owned executor reaches its absolute runtime ceiling during drain, the old worker records `deploy_budget_expired` and requests a typed checkpoint/requeue rather than routine deploy SIGTERM. If the owner is lost, the preview URL remains durable but the preview becomes `unavailable` and restartable instead of exposing worker-token internals. Extra worker generation ports are required for non-blocking preview-preserving deploys because every advertised worker host port must be reachable from app nodes. Without an extra safe port, routine deploy fails closed and operators must run an explicit maintenance path for disruptive endpoint reuse. See [implemented/44-sandbox-preview-server.md](implemented/44-sandbox-preview-server.md), [implemented/48-worker-owned-preview-routing.md](implemented/48-worker-owned-preview-routing.md), [implemented/77-durable-preview-start.md](implemented/77-durable-preview-start.md), and [future/88-preview-runtime-ownership-drain.md](future/88-preview-runtime-ownership-drain.md).
    ...
    - Preview dependency installation is a first-class optional phase (`preview.install`) between infrastructure init and service startup. 143 computes a lockfile/config/runtime cache key, stores success markers under `.143/cache/preview-install/`, cleans only repo-declared `clean_paths`, and surfaces install failures as `PREVIEW_INSTALL_FAILED` with persisted startup log tails. Cache-restored install skips require repo-declared `verify_paths` to point at cache-restorable workspace artifacts; platform-owned `.143/cache` verify paths are ignored for that restore-satisfaction check for compatibility with existing configs. Repos should move package-manager install logic out of preview service commands when they need platform-owned install reuse and cleanup.
    - Session preview cold starts can restore two preview path-cache kinds while the hydrated session snapshot remains authoritative for source files. Dependency artifact caches are WorkDir-relative and remain gated by the install marker; effective paths are `preview.install.clean_paths + preview.install.cache.paths + inferred paths from known JavaScript/Python/Go dependency files`, excluding platform-owned `.143/cache` paths. Package-manager global caches are HomeDir-relative, inferred from lockfiles/install command (npm, pnpm, yarn, bun, pip, uv, poetry, Go) plus optional `preview.install.cache.package_manager.paths`, and restore before install even when the marker is absent. Blobs are keyed by cache kind, lockfile contents, install command/cwd, runtime provider/image, effective paths, and package-manager set, with scheduler locality improved by cache-location hints and rendezvous hashing across both cache kinds. Restores validate blob checksums and tar members on the worker, stage compressed blobs on host-backed worker-local cache disk instead of small tmpfs paths, stream extraction into the sandbox, and force a normal install after dependency-artifact restore failure instead of trusting an existing marker. Package-manager restore failures only continue cold. Cache-holder scheduling is load-aware, choosing the least-loaded matching holder before falling back to recency. Low-priority `preview_cache_prewarm` jobs can opportunistically populate both cache kinds for branch/PR preview targets and snapshot-producing session turns before a user opens a preview, gated by `PREVIEW_CACHE_PREWARM_ENABLED`. See [implemented/93-session-preview-dependency-cache.md](implemented/93-session-preview-dependency-cache.md).
...
- Preview startup should keep orchestration milestones and service/container stdout and stderr available as an optional, hidden-by-default log stream in the Preview tab. Pre-install/cache phase transitions and `preview.install` output should also be persisted to `preview_logs` so cold-start silence is explainable before application services emit logs. The default startup canvas stays phase-focused, but users can expand raw preview logs to understand sandbox acquisition, branch clone/checkout, infrastructure containers, cache restore/save, installs, migrations, compiles, or readiness delays while the preview is still starting.
- Preview operations have a compact internal Grafana dashboard for the core health signals: active previews, startup p50/p95, ready vs failed/unavailable counts, cache restore hit rate, and recent preview errors. It stays on the existing VictoriaLogs path through low-volume structured lifecycle samples and cache restore events; see [implemented/99-preview-health-dashboard.md](implemented/99-preview-health-dashboard.md).
- Connected preview secrets use preview secret bundles managed from Settings → Preview. Repo config may reference a bundle by name, allowlist env vars or generated file paths, and target services; runtime launch decrypts the bundle for the active org/repository and delivers values as process environment variables or writes pasted private file contents into the preview workspace. The Preview settings page manages auto-preview policy and repo-scoped preview secret bundles; preview-only API token management is deprecated in favor of External API tokens. See [implemented/88-preview-settings-page.md](implemented/88-preview-settings-page.md).
- Agent runtime credentials should be managed as visible, ordered auth stacks rather than hidden single-provider settings. The settings UX is a prioritized list of coding agent auths with an explicit default and fallback order across personal and organization scopes; see [implemented/57-coding-agent-settings-rethink.md](implemented/57-coding-agent-settings-rethink.md).
    ...
    - OpenCode is a first-class coding-agent runtime using the same auth-stack primitives with explicit `opencode` credentials, backing-provider/model compatibility checks, cost-first model choices, and `opencode run --format json` execution; detailed notes live in [implemented/95-opencode-agent-adapter.md](implemented/95-opencode-agent-adapter.md).
    - Coding-agent credential config and runtime health are separate insert-only active-row versioned concepts. `coding_credentials.id` remains the stable logical id, config versions live under `coding_credentials.version_id`, and runtime status/rate-limit/verification state lives in `coding_credential_runtime_state`; see [implemented/91-versioned-coding-credentials-runtime-state.md](implemented/91-versioned-coding-credentials-runtime-state.md).
    - The unified pair is the ONLY storage for coding-agent credentials: the dual-write mirror, org_credentials coding rows, user_credentials coding rows + `is_team_default`, and the legacy `/settings/coding-auths` + `/settings/credentials/{personal,team,resolved}` APIs were retired by the unified-credentials cleanup (PR 5); `user_credentials` survives solely for GitHub user OAuth tokens. See [future/65-unified-coding-credentials.md](future/65-unified-coding-credentials.md).
    - Coding-agent auth stacks persist temporary rate-limit health in the versioned runtime state and derive `rate_limited` UI/API status without changing the stored credential status. Runtime credential selection skips future-limited auths across workers, preserves the personal-before-org fallback order for Codex, Claude Code, Gemini, Amp, Pi, and future coding agents, and makes continued sessions resolve fresh auth before each turn; see [implemented/78-coding-agent-rate-limit-fallback.md](implemented/78-coding-agent-rate-limit-fallback.md).
...
- **Agent log provider tools**: Sandbox agents use the shared `143-tools logs` command namespace (`logs query`, `logs context`, `logs fields`, and provider-supported `logs stats`) with an optional `--provider` flag. The implementation is provider-neutral, supports VictoriaLogs and Mezmo adapters, enforces bounded time windows and response caps, redacts/caps returned payloads, signs pagination cursors, and keeps provider-native query syntax instead of inventing a universal log language. See [90-agent-log-provider-tools.md](implemented/90-agent-log-provider-tools.md).
