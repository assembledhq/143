...
- Verified-domain auto-join lets admins prove domain ownership with a DNS TXT record and then admit users whose OAuth provider asserts a verified email on that domain. Password signup does not auto-join from a typed email address until first-party email verification exists. Detailed design: [implemented/92-verified-domain-auto-join.md](implemented/92-verified-domain-auto-join.md).
