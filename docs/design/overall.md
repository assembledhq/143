...
- When Linear is actively integrated, the manual-session composer should expose an `Add linear issue` affordance in the add menu and render added issues as removable chips in the composer instead of dumping pasted Linear refs back into the raw textarea. The action should stay hidden when Linear is not connected.
    ...
    - Session tabs/threads now use a **durable per-thread inbox plus live per-thread runtime handles**. The inbox is the platform's source of truth for ordering, audit, recovery, and backpressure, while the runtime registry tracks live owner routing and delivery/ack cursors for active threads. The current adapter boundary still acks at thread turn boundaries rather than arbitrary mid-turn prompt injection, but the durable control-plane contract is now implemented. Detailed design: [implemented/75-thread-runtime-handles-and-durable-inbox.md](implemented/75-thread-runtime-handles-and-durable-inbox.md).
