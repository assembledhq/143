...
- Agent runtime credentials should be managed as visible, ordered auth stacks rather than hidden single-provider settings. The settings UX is a prioritized list of coding agent auths with an explicit default and fallback order across personal and organization scopes; see [implemented/57-coding-agent-settings-rethink.md](implemented/57-coding-agent-settings-rethink.md).
    ...
    - The usage page now supports **execution analytics by agent, model, reasoning, and capacity** on top of the billing rollups. Org totals still read from `usage_hourly`, while a separate `usage_hourly_execution` rollup powers execution filters, breakdown tables, CSV exports, and stacked token-by-model charts without raw-session scans. That execution rollup stores both capacity-specific rows and synthetic all-capacities rows so agent/model/reasoning views and filtered exports can report exact per-hour sessions and concurrency without double-counting capacity changes. See [implemented/66-usage-breakdown-by-agent-model-reasoning.md](implemented/66-usage-breakdown-by-agent-model-reasoning.md).
