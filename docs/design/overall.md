...
- Branch previews are first-class preview targets keyed by repository, branch, commit SHA, and config name, with stable app-owned links and a create flow outside session detail. Session and PR preview surfaces now resolve through this branch-preview control plane rather than treating runtime instances as the durable user-facing artifact. Detailed design: [implemented/83-branch-and-pr-previews.md](implemented/83-branch-and-pr-previews.md).
