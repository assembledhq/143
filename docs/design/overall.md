...
- **Agent log provider tools**: Sandbox agents use the shared `143-tools` log command set (`log_query`, `log_context`, `log_fields`, and provider-supported `log_stats`) with an optional `--provider` flag. The implementation is provider-neutral, supports VictoriaLogs and Mezmo adapters, enforces bounded time windows and response caps, redacts/caps returned payloads, signs pagination cursors, and keeps provider-native query syntax instead of inventing a universal log language. See [90-agent-log-provider-tools.md](implemented/90-agent-log-provider-tools.md).
