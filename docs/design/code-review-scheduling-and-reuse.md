...
- The 143 UI exposes **Policy → Safeguards → Review scheduling**, **Review now** on executed review rows, and a dedicated **Queue** tab with pause/resume, wait reasons, and 25-row cursor pagination. When scheduling is enabled, the rolling GitHub comment includes **Review now**, linking to `/code-reviews?review_now=<source-session-id>`. The authenticated page loads that session in the active organization and shows the PR before a separate button click submits the existing request POST. Link visits and previews never mutate review state. Superseded comments omit the action. Viewers can read the queue; admins/members can request or pause. Request identity survives an uncertain HTTP response while the control remains mounted.
