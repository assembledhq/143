...
- Workers advertise `static_egress_capable=true` only when platform config enables static egress, the customer-facing public IP is configured, and host reconciliation has written `STATIC_EGRESS_CAPABILITY_FILE` after installing and probing the WireGuard policy route.
...
- The static egress bridge has a separate dnsmasq sidecar and resolver file.
