...
1. Keep the settings surface named **Preview**.
2. Make **Preview secrets** the primary use case on the page.
3. Use the same inventory-table pattern already used for coding-agent auths, personal credentials, and usage breakdowns.
4. Preserve the repo-scoped backend model for preview secret bundles.
5. Avoid exposing secret values in list views, API responses, logs, audit events, or browser-visible config.
...
4. Creating or managing preview-only API tokens. External API tokens are the supported path for preview API access.
...
1. **Auto-preview**
   - Repository-level automatic preview policy.
   - Organization pool capacity control for concurrent auto-previews.
...
Preview-only API token management was removed from this page. The legacy `/api/v1/previews/api-tokens` management endpoints return `410 Gone` with a deprecation message that points callers to External API tokens.
...
```text
Preview
Configure auto-preview policy and preview secrets.

Auto-preview
┌─────────────────────────────────────────────────────────────────────────────┐
│ Repository      Mode                   Open PRs        Updated             │
├─────────────────────────────────────────────────────────────────────────────┤
│ assembled/143   Off / Warm / On        12              Jun 8, 2026         │
└─────────────────────────────────────────────────────────────────────────────┘
...
- Use tabs for `Auto-preview` and `Secrets`.
- In `Secrets`, use a section header outside cards: `Preview secrets` with a `New bundle` button.
...
- Use `Badge` for output summaries and statuses.
- Use `Button` with lucide icons for actions (`Plus`, `KeyRound`, `TestTube2`, `Pencil`, `Trash2`).
- Use `EmptyState` inside the table/card area when there are no bundles.
...
Preview secret bundles are currently repo-scoped. The settings page must include a repository selector inside the Preview secrets section.
...
Deprecated preview-only API token management routes:

- `GET /api/v1/previews/api-tokens` — returns `410 Gone`
- `POST /api/v1/previews/api-tokens` — returns `410 Gone`
- `DELETE /api/v1/previews/api-tokens/{token_id}` — returns `410 Gone`
...
- Preview-only API token management is deprecated. Existing token authentication may remain temporarily for compatibility, but new token creation must go through External API tokens.
...
10. Preview API token tab and runtime-settings callout are not rendered on `/settings/previews`.
11. The page does not fetch `/api/v1/previews/api-tokens`.
12. Mobile rendering uses stacked rows and does not expose a desktop-only table as the only readable UI.
13. Deprecated preview API token management endpoints return `410 Gone`.
