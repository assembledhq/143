...
Worker deploys treat `preview_internal_base_url` as owned while a matching `nodes` row is active or draining with a fresh heartbeat. This is separate from Docker listener checks and active `preview_runtimes` leases: a port that has just become locally free can still be advertised by the old generation until the dead-node timeout catches up. Routine blue/green deploys therefore refuse to reuse that endpoint and choose another configured worker port, preventing app-side preview RPC auth checks from signing for one node while a newer generation answers at the same URL.
