...
App nodes also treat gateway-to-worker reachability as part of the runtime health contract. The preview gateway marks the specific active runtime `unavailable` with `unavailable_reason = worker_endpoint_unreachable` when proxying to the runtime endpoint fails, and an app-side reachability monitor periodically dials active runtime endpoints to catch ready-but-undialable runtimes before a user opens the preview. This keeps the durable preview URL restartable while avoiding stale `ready` rows whose worker heartbeat is fresh but whose endpoint cannot be reached from the app tier.
