...
The branch-only action has its own publish state in `session_publish_state` (`branch_creation_state`, `branch_creation_error`, `branch_url`) instead of overloading `pr_creation_state`. This keeps a branch-only publish from blocking a later `Create PR` action on the same session.
