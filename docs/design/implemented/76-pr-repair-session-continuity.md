...
Only one active repair should run for a given PR health version and action type.
...
- if PR health advances mid-command, let the running turn finish but stop suppressing CTAs for the newer health version
