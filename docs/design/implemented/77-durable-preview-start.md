...
If a `start_preview` job reaches local sandbox capacity on its pinned worker, the worker retries the job without consuming normal attempts and retargets it to another active worker that advertises free sandbox capacity. When no worker advertises space, the retry clears the target pin so the next free worker can claim it instead of looping on the saturated node until the retry window expires. When a different worker claims the retry, it reassigns both `preview_instances.worker_node_id` and the active `preview_runtimes` endpoint before launching the provider, so gateway traffic follows the worker that actually owns the running container.
