...
- OpenRouter open-source model choices must have audited US inference-provider routes. As of design 115 the audited allowlist lives on each OpenRouter route in the logical-model registry (`internal/models/opencode_models.go`, `OpenCodeRoute.USProviderList`); `openCodeOpenRouterModelConfigs` reads it via `models.OpenCodeUSProviderList` and emits the per-model `options.provider` block that sets `only` and `order` to audited provider slugs, disables fallbacks beyond that allowlist, denies provider data collection, and requires parameter support. Prefer two or three high-quality audited providers per model where OpenRouter exposes them; document any single-provider exception in code.
