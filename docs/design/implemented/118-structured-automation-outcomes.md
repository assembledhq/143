...
Migration 246 performs a conservative one-time backfill from Main-thread summaries whose entire leading token matches the legacy `#<pr>: pass|reject|advise|skipped — ...` format. It maps those tokens to typed outcomes and marks them `legacy_inferred`. It does not inspect session-level `REVIEW_CLEAN`, invent external actions, or classify any other free-form summary.
