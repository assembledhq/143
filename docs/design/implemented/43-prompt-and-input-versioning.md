...
Migration 260 exposes the neutral `reference_documents` and
`reference_context_set_*` names as automatically updatable views over the
legacy tables during the rolling-compatibility window. Application code uses
the neutral contract; physical legacy-table removal is a later contraction.
...
- **PM Documents** (`reference_documents` table) store current content. Updates overwrite in place. There is no history of what the PM agent was reading at any given point.
...
`reference_documents` stores the current content of each document. The `Update` method overwrites `title`, `content`, `doc_type`, etc. in place. Previous content is lost.

#### Approach: Insert-only versioning on `reference_documents` itself
...
**Schema changes to `reference_documents`:**
...
CREATE UNIQUE INDEX idx_reference_documents_active_logical
    ON reference_documents (org_id, logical_id) WHERE active = true;
```
...
2. On update, within a transaction (same pattern as `MemoryStore.UpdateMemoryAndGet`):
   - `UPDATE reference_documents SET active = false WHERE id = @id AND org_id = @org_id AND active = true RETURNING ...`
...
4. Version history = `SELECT * FROM reference_documents WHERE org_id = @org_id AND logical_id = @logical_id ORDER BY created_at DESC`.
...
```sql
CREATE TABLE reference_context_set_pins (
    id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    org_id          UUID NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
    created_at      TIMESTAMPTZ NOT NULL DEFAULT now()
);

CREATE TABLE reference_context_set_pin_members (
    pin_id          UUID NOT NULL REFERENCES reference_context_set_pins(id) ON DELETE CASCADE,
    document_id     UUID NOT NULL REFERENCES reference_documents(id) ON DELETE RESTRICT,  -- points to specific version row
...
- Since `reference_documents` rows are never deleted (insert-only), the pin references are stable forever.
...
|-------|--------|---------|
| Document created | `reference_document.created` (existing) | `{ "document_id": "...", "logical_id": "..." }` |
| Document updated | `reference_document.updated` | `{ "document_id": "...", "logical_id": "...", "previous_id": "..." }` |
| Document restored to old version | `reference_document.restored` | `{ "document_id": "...", "logical_id": "...", "restored_from_id": "..." }` |
...
```json
{
  ...
  "reference_context_set_pin_id": "uuid-of-document-set-pin",
...
GET    /api/v1/reference-documents/:id/versions        -- version history for a document
POST   /api/v1/reference-documents/:id/restore         -- restore to a specific version
...
1. **Alter `reference_documents`** — Add `active`, `logical_id`, `content_hash` columns. Backfill: set all existing rows to `active = true`, `logical_id = id` (each existing doc becomes its own first version), compute `content_hash`.
2. **Add `reference_context_set_pins` and `reference_context_set_pin_members`** tables.
...
4. **Update `ReferenceDocumentStore.Update`** — Replace in-place UPDATE with insert-only transaction pattern (deactivate + insert), following the same structure as `MemoryStore.UpdateMemoryAndGet`.
...
**Audit logs**: The existing `AuditEmitter` and `AuditResourceReferenceDocument` resource type are extended with new actions for version tracking. Audit logs reference version IDs in their `details` JSONB but do not store content — that lives in the version rows, which have no retention expiry.
