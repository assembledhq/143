...
> PR content generation also includes bounded summaries from all visible session threads so a recently-created review loop is treated as one context signal, not as the whole PR story. The diff, files changed, issue/session title, and full thread set remain the reviewer-facing source of truth.
