...
5. The GitHub API health dashboard lives at `deploy/grafana/provisioning/dashboards/github-api-health.json`. It uses the canonical `_msg:"github api request"` event emitted once per actual server-owned GitHub HTTP API request. Endpoint paths are normalized before logging; authorization headers, tokens, response bodies, and query values are never logged. The event carries request result and latency plus rate-limit limit, remaining, used, reset, resource, retry-after, auth type, installation ID, bounded caller, sync reason, and rollout mode. Installation cooldown decisions emit `_msg:"github request deferred"` separately with `github_request_suppressed` distinguishing enforce-mode suppression from observe-mode prediction, so neither inflates API request counts. The dashboard attributes throttles and cooldown decisions by installation, resource, route, caller, and sync reason and shows deferrals by rollout cohort. The bounded `github.principal.unresolved` counter and matching `github_principal_unresolved:true` request field identify covered calls whose principal was not explicitly resolved; any such event blocks canary expansion. The controller intentionally excludes git protocol traffic, user OAuth and app-JWT calls from enforcement, and calls made directly inside sandboxes.
   Controller-covered installation clients are the GitHub App/PR services, production code-review submitter, integration repository sync/list/claim client, and team user-search client. Auth/OAuth handlers, GitHub App user auth, identity user lookups, trigger setup, app-JWT token exchange, and direct sandbox/git traffic are intentionally uncovered because they use a different limiting principal or execute outside the server controller. A resolved `app_installation` field is not by itself evidence of controller participation; canary review must confirm both explicit identity and controlled-client construction for every covered row.
