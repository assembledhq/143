...
Status: The 2026-09-24 implementation and rollout history is recorded below. The 2026-09-25 routing correction in decision 16 is being implemented locally; it is not yet deployed. Current production rollout state is not asserted by this document. Last reviewed: 2026-09-25.
...
16. **2026-09-25, explicit user correction: Code and review policy determine recheck compatibility.** With a complete valid baseline, only the code fingerprint or the versioned resolved review policy (ID, version, and configuration digest) invalidates code coverage. Title, description, generated attribution, discussion, request, CI, and eligibility changes are current evidence/gates. Auxiliary prompt/runtime fingerprints remain immutable provenance and freshness inputs. A failed evidence attempt can retry against the same baseline. Model uncertainty and capture/execution failures must not silently launch a fresh panel. Explicit full-review requests and missing usable baselines retain their full-review path. This supersedes the intent and auxiliary-contract invalidation rules in decisions 3, 14, and 15 and the earlier delivery-slice acceptance descriptions below.

17. **2026-09-25, Fable follow-up: Settle failed captures and preserve legacy evidence provenance.** Deterministic capture limits and transient failures lasting 15 minutes record a failed request, release that request's pending intent, and stop its wake/repair loop without disturbing newer requests or active reviews. Manual requests receive `409 CODE_REVIEW_RECHECK_UNAVAILABLE`; a new click gets a new request ID, while uncertain transport failures retain the old ID. Trusted mentions acknowledge the recorded failure. Superseded, provably unsent publications close their repair marker when replacement capture fails. No full panel is launched by these failures. Manifest v3 remains compatible: new captures set `text_evidence.full_discussion_captured`; its absence means old discussion prose was not fully archived. Discussion citations alone cannot establish new evidence against those older baselines, including prose moved into an evidence section. Newly verified description, check, or image evidence can still resolve a blocker. No historical manifest or finding is rewritten.
...
| Same head, base SHA/ref and changed-file manifest, complete baseline, unchanged versioned review policy; any evidence/context/gates changed | `evidence_only`; continue or reconstruct the orchestrator |
| Head/base/ref or changed-file manifest changed, full baseline unavailable/incomplete, versioned review policy changed, or explicit force-fresh request | `full`; start a fresh conversation generation and run the normal panel |
...
- Captured policy ID, version, and configuration digest match. Review instructions, roster, and approval rules are part of that configuration. Prompt/runtime fingerprints remain provenance and freshness inputs, not independent full-review triggers.
...
- PR title, description, generated attribution, discussion, and request text may change. Capture their complete untrusted contents for the recheck; evidence sections are extraction conveniences rather than reuse boundaries. Unsupported Markdown remains exact source text. Changes to these sources trigger evidence reassessment, never code-review invalidation.
...
Implement a pure routing function over captured facts, the evidence reassessment response schema and fail-closed validator, baseline-reference assembly, human-attention escalation, fresh gate evaluation, and shared assessment publication helpers. Distinguish the code baseline from the latest publication predecessor; reconcile summary and formal approval independently. Add a `.template` under `internal/prompts/templates/` and an exported render function. Budget the complete rendered message, including any embedded reconstruction history, against the existing thread-message bound; separately enforce the selected provider's context budget. Missing baseline coverage requires a full review. Oversized continuation context fails the recheck without silently dropping reviewers/findings or dispatching a panel.

Exit: same-code screenshot, test-log, and evidence-resolved finding fixtures dispatch one orchestrator execution and zero reviewer executions; the real screenshot is available to the model and cited by ID. Changed code or versioned review policy and incomplete baselines select full review. Changed intent is reassessed as evidence. Native and reconstructed execution produce the same allowed response contract, and publication handles races/idempotency.
...
| New evidence reveals a concern | Concern retained for human review, no automatic full panel and no approval from the narrow result |
| Changed head/base/ref/file manifest or versioned policy; legacy/incomplete baseline | Full route with a recorded reason |
| Changed title, description, generated attribution, discussion, requests, or CI | Evidence recheck with fresh approval gates |
...
- The evidence-section intent parser and human-source discovery require adversarial heading, fence, deletion, overflow, and prose-change tests. Changed unsupported or ambiguous sources and substantive intent are captured as evidence; they do not invalidate unchanged code coverage. Capture failures remain visible failures rather than silently starting a full panel.
