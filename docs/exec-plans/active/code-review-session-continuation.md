...
Status: PR #2178 is open; evidence reassessment now covers requirements and original findings following the 2026-09-24 scope correction. CI passed on b549a91e; the second Fable review is complete and validated follow-up fixes pass local verification. Final-head CI and re-review are pending. Activation remains gated on provider, browser, CI, and pilot validation. Last reviewed: 2026-09-24.
...
- PR title and non-evidence statement of intent are unchanged. Ordinary Testing, Evidence, Validation, Logs, and Benchmarks sections can change as evidence. Non-evidence prose, including substantive human discussion, remains part of intent. The parser must respect Markdown heading/fence boundaries and reject ambiguous input. A section label does not authorize a new intended behavior: the orchestrator escalates if content changes intent.
...
- The evidence-section intent parser and human-source discovery require adversarial heading, fence, deletion, overflow, and prose-change tests. Unsupported or ambiguous input and substantive intent changes route full.
