...
143 maps the Linear creator to an organization member using a connected identity or an exact provider-attested email. When no trusted mapping exists, the run is labeled as a team session and uses organization defaults. Linear posts a short-lived **Connect account** link so the creator can claim future personal attribution, credentials, and approvals. Admins can review Linear mappings and unmatched creators in **Team settings → External identities**.
</Callout>
