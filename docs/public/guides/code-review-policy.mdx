...
Every reviewer and the orchestrator receive the same immutable image snapshot for the assessed commit. Image pixels, filenames, alt text, captions, and nearby discussion are always untrusted pull-request content: 143 uses them as factual evidence but never as instructions or as independent reasons to block approval. An image-backed passing assessment cites the captured evidence ID. A preview link or repository-native visual artifact can also satisfy a visual requirement when the assessment states that basis; description text or diff text alone cannot.

Open the review's **Evidence** panel to see captured thumbnails, source surface and author, status, failure details, and which images the orchestrator cited. Inaccessible, unsupported, or over-limit images remain visible as provenance but cannot satisfy a requirement. A review uses the snapshot taken when that assessment began; request 143 again if description or comment edits on the same commit should be evaluated in a new assessment.
