...
The **Queue** tab shows pending work before a session exists, including draft,
...
It still requires an open, ready PR and an enabled review policy. It waits for
...
second assessment for every click. Draft and closed PRs cannot start new review
work. Historical approval still stops automatic re-review; use an explicit
request when another review is needed after approval.
