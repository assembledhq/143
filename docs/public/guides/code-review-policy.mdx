...
Open the review's **Evidence** panel to see captured thumbnails, source surface and author, status, failure details, and which images the orchestrator cited. 143 retains per-image provenance for the first 32 images across the description and human discussion; the panel reports only an aggregate omitted count for any later images. Inaccessible, unsupported, or oversized retained images remain visible as provenance but cannot satisfy a requirement. A review uses the snapshot taken when that assessment began; later description or comment edits are not retroactively included. Same-head **Review now** can reuse an existing assessment and does not guarantee a fresh evidence snapshot.

Where durable review scheduling is enabled, an explicit reviewer request ensures review of the current revision and joins or reuses existing applicable work. A new request with different review context, or following a deterministic early stop, can still require another assessment. In installations using the legacy scheduler, each genuinely new explicit request starts a distinct assessment, even on the same commit. Redelivery of an existing request keeps its original identity. The Code reviews page retains executed assessments as audit history.

### Re-check updated evidence

When **Allow PR re-checks** is enabled, choose **Re-check PR evidence** from a completed review's overflow menu or follow the link in its review comment. With a complete prior review, unchanged code and an unchanged review policy, 143 reassesses screenshots, test results, logs, descriptions, discussion, and existing findings without rerunning the code reviewers. Updates to the title, generated attribution, or CI status also use this evidence path.

**Any code change requires a new full review.** Changes to the versioned review policy also require a full review. A missing or incomplete prior review must establish a full baseline first. Use **Request Full Re-Review Now** when you explicitly want a new code review of unchanged code.

An evidence recheck does not guarantee approval. 143 preserves unresolved findings, checks current approval gates, and requires evidence to resolve a blocker. Missing or invalid evidence remains blocking. If the recheck cannot complete or raises a new concern requiring human judgment, it reports that condition without automatically starting another code review.
...
When durable scheduling is enabled for your installation, admins can open
**Policy → Safeguards → Review scheduling** to configure:

- **Automatically re-review changed PRs**: monitor PRs that have already been requested for review.
- **Wait after changes**: wait for pushes to settle before automatic review starts.
- **Minimum interval between automatic reviews**: space reviews from the first actual reviewer start, including an explicitly requested review.

Both timing requirements must be satisfied. Repeated delivery of the same
revision does not restart the wait. Existing settings default to a one-minute
wait and no additional minimum interval; **Use 5-minute wait and 15-minute
interval** applies those values explicitly. These controls autosave for the
organization. The **Dispute reassessment cooldown** is separate and applies to
repeated objections.

The **Queue** tab shows pending work before a session exists, including timing,
paused, active-review, and GitHub availability waits. It displays 25 PRs per page
with Previous and Next controls, separately from the executed activity on the
**Reviews** tab. Members and admins can pause or resume automatic monitoring
for a pending PR.

**Review now** bypasses the two timing delays and an automatic-monitoring pause.
It still requires an open PR and an enabled review policy. It waits for
active work and can reuse a completed assessment of the same head and base under
the current policy. It does not resume automatic monitoring or guarantee a
second assessment for every click. Draft PRs support automatic reviews, Review
now, retries, and dispute reassessments under the same scheduling rules.
Converting a PR to draft does not stop an active review. Closed PRs cannot start
new review work. Historical approval still stops automatic re-review; use an
explicit request when another review is needed after approval.

Content reuse across rebases or restacks, review-specific usage budgets, reserved
interactive capacity, and forced fresh review are not part of this scheduling
release. Scheduling is available automatically when the installation's GitHub
review service is configured; no separate rollout flag is required.
...
Under **Safeguards → Approval criteria**, **Additions** and **Deletions** set independent maximum line counts for automatic approval. A PR must stay at or below both limits; added and deleted lines are not combined. Both default to 300. Existing policies carry their former **Lines changed** value into each limit, which admins can then adjust separately. Exceeding either limit requires human review and links to the corresponding setting.
