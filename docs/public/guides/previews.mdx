...
<Screenshot
  ...
  alt="143 session preview panel showing a running preview booting with infrastructure, services, and preview steps"
  width={1440}
  height={810}
...
`restored`, `reset`, or `unavailable` when restoration cannot be guaranteed.

The session preview panel displays that same platform-owned browser rather than
opening a second browser context. It is watch-only while the agent has control.
Select **Take control** to acquire an audited five-minute lease for clicking,
typing, scrolling, or completing login and MFA in the shared browser, then
select **Return control** so the agent continues from the exact URL and storage
state you left behind. Agent input is paused during human control, and abandoned
leases expire automatically.

Screenshot and multi-viewport responses include a `capture.url` served through the normal upload pipeline. During the current rollout the same reference is also repeated under the former `artifact` key; new integrations should read `capture`. Use `--inline-base64 false` when agents do not need PNG bytes in stdout, or `preview observe --output path.png` to write an observation directly into the workspace.
...
Do not cache source directories, secret files, `.git`, or `.143/cache/preview-install`. Use pinned lockfiles when possible; mutable tags such as `latest` can leave stale dependency outputs.
