...
Screenshot and multi-viewport responses include an `artifact.url` served through the normal upload pipeline. Use `--inline-base64 false` when agents do not need PNG bytes in stdout, or `preview observe --output path.png` to write an observation directly into the workspace.
