...
- Backups for Postgres and `/var/lib/143`.
...
The host preparation script creates the Docker networks, sandbox resolver config, sandbox-auth socket directory, preview dependency cache directory, and `/var/lib/143` data directories. It also prints the host Docker socket group id required by `DOCKER_GID`.
...
- Local storage under `/var/lib/143` must be backed up with Postgres.
...
Before routing real repository work through a single-node install, test restoring the Postgres volume, `/var/lib/143`, and `.env.single-node` onto a fresh host.
</Callout>
